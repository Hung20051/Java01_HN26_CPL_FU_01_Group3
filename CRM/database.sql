...
-- ================================================================
-- TECHNICIAN MODULE MIGRATION
-- Run this on top of existing crm_db
-- ================================================================

USE crm_db;

-- 1. Thêm labor_cost + notes vào work_assignments
ALTER TABLE work_assignments
    ADD COLUMN  labor_cost DECIMAL(15,2) DEFAULT 0 COMMENT 'Tiền công của technician này',
    ADD COLUMN  notes      TEXT          DEFAULT NULL COMMENT 'Ghi chú của technician';

-- 2. Bảng repair_reports — báo cáo sửa chữa (1 per work_task)
CREATE TABLE IF NOT EXISTS repair_reports (
    id                 INT AUTO_INCREMENT PRIMARY KEY,
    work_task_id       INT           NOT NULL UNIQUE,
    service_request_id INT           NOT NULL,
    technician_id      INT           NOT NULL,
    ...
    FOREIGN KEY (technician_id)      REFERENCES users(id),
    FOREIGN KEY (invoice_id)         REFERENCES invoices(id),
    INDEX idx_sr   (service_request_id),
    INDEX idx_tech (technician_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- 3. Bảng repair_report_parts — parts đã dùng trong 1 báo cáo
CREATE TABLE IF NOT EXISTS repair_report_parts (
    ...
    unit_price   DECIMAL(15,2) NOT NULL DEFAULT 0,
    total_price  DECIMAL(15,2) NOT NULL DEFAULT 0,
    FOREIGN KEY (report_id)    REFERENCES repair_reports(id) ON DELETE CASCADE,
    FOREIGN KEY (part_type_id) REFERENCES part_types(id),
