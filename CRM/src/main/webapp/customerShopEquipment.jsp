...
<head>
    ...
    <style>
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .shop-tabs{display:flex;background:white;border-radius:10px;border:1px solid #e2e8f0;overflow:hidden;margin-bottom:20px;width:fit-content}
        ...
        /* Equipment cards - list style vì thiết bị lớn */
        .product-list{display:flex;flex-direction:column;gap:14px;margin-bottom:24px}
        .equip-card{background:white;border-radius:12px;border:1px solid #e2e8f0;padding:18px 20px;display:flex;align-items:center;gap:18px;transition:.2s}
        .equip-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.08)}
        .equip-icon{width:64px;height:64px;background:linear-gradient(135deg,#fce7f3,#dbeafe);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.8rem;flex-shrink:0}
        .equip-info{flex:1}
        .equip-cat{font-size:.7rem;font-weight:600;color:#6366f1;text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px}
        .equip-name{font-size:1rem;font-weight:700;color:#1e293b;margin-bottom:4px}
        .equip-desc{font-size:.82rem;color:#94a3b8;margin-bottom:6px}
        .equip-stock{font-size:.76rem;color:#10b981;font-weight:600}
        .equip-stock.low{color:#f59e0b}
        .equip-right{text-align:right;flex-shrink:0}
        .equip-price{font-size:1.2rem;font-weight:800;color:#ef4444;margin-bottom:10px}
        .btn-add-equip{padding:10px 20px;background:#6366f1;color:white;border:none;border-radius:9px;font-size:.875rem;font-weight:600;cursor:pointer;transition:.15s;display:flex;align-items:center;gap:6px}
        .btn-add-equip:hover{background:#4f46e5}
        .equip-note{font-size:.72rem;color:#94a3b8;margin-top:6px}
...
<aside class="sidebar">
    ...
    <a href="<%= ctx %>/customerShop?action=equipment" class="nav-item active"><i class="fas fa-server"></i> Thiết bị</a>
...
<main class="main">
    ...
    <div class="shop-tabs">
        <a href="<%= ctx %>/customerShop?action=parts"     class="shop-tab"><i class="fas fa-puzzle-piece"></i> Linh kiện</a>
        <a href="<%= ctx %>/customerShop?action=equipment" class="shop-tab active"><i class="fas fa-server"></i> Thiết bị</a>
    </div>

    <form method="get" action="<%= ctx %>/customerShop">
        <input type="hidden" name="action" value="equipment">
        <div class="toolbar">
            <input class="search-input" type="text" name="keyword" placeholder="Tìm kiếm thiết bị..." value="<%= keyword %>">
    ...
    <div class="result-info">Tìm thấy <strong><%= total %></strong> loại thiết bị</div>

    <% if (items.isEmpty()) { %>
    <div class="empty-state"><i class="fas fa-server"></i> Không có thiết bị nào phù hợp</div>
    <% } else { %>
        ...
        <div class="equip-card">
            ...
            <div class="equip-info">
                ...
                <div class="equip-stock <%= item.availableQty <= 2 ? "low" : "" %>">
                    <i class="fas fa-check-circle"></i> Còn <%= item.availableQty %> unit<%= item.availableQty <= 2 ? " (sắp hết)" : "" %>
                </div>
            </div>
            <div class="equip-right">
                ...
                <form method="post" action="<%= ctx %>/customerShop">
                    ...
                    <button type="submit" class="btn-add-equip"><i class="fas fa-cart-plus"></i> Thêm vào giỏ</button>
                </form>
                <div class="equip-note">Mỗi lần chỉ mua 1 unit (có serial number riêng)</div>
            </div>
        </div>
        <% } %>
    </div>
