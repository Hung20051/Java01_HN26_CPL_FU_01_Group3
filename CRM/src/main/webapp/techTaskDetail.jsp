...
<head>
    ...
    <style>
        :root {
            ...
            --sb-accent-2:#a5b4fc; --sb-item-on:rgba(129,140,248,0.2); --sb-width:252px;
            --bg:#f3f4f9; --bg-card:#fff; --bg-topbar:#fff;
            ...
            --blue:#2563eb; --teal:#0d9488; --purple:#7c3aed; --orange:#ea580c;
        }
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sb-item i{width:28px;height:28px;display:flex;align-items:center;justify-content:center;
            font-size:.78rem;border-radius:8px;background:rgba(255,255,255,.06);flex-shrink:0}
        ...
        .card{background:var(--bg-card);border:1px solid var(--border-light);border-radius:16px;
            overflow:hidden;margin-bottom:18px;box-shadow:0 1px 6px rgba(0,0,0,.05);
        ...
        .desc-box{background:#fafbff;border:1px solid var(--border-light);border-left:3px solid var(--primary-2);
            border-radius:12px;padding:14px 16px;font-size:.83rem;color:var(--text-b);
        ...
        .form-row input,.form-row textarea,.form-row select{
            width:100%;padding:10px 13px;border:1.5px solid var(--border-light);border-radius:9px;
            font-size:.83rem;font-family:'Sora',sans-serif;color:var(--text-b);
            background:#fff;outline:none;resize:vertical;transition:all .2s}
        .form-row input:focus,.form-row textarea:focus,.form-row select:focus{
            border-color:rgba(79,70,229,.4);background:#faf9ff;box-shadow:0 0 0 3px rgba(79,70,229,.07)}
        ...
        .part-row{display:grid;grid-template-columns:1fr auto auto auto;gap:8px;align-items:center;
            background:#fafbff;border:1px solid var(--border-light);border-radius:10px;padding:10px 12px}
        .part-row select,.part-row input{padding:6px 10px;border:1.5px solid var(--border-light);
            border-radius:7px;font-size:.8rem;font-family:'Sora',sans-serif;background:#fff;outline:none}
        .part-row select:focus,.part-row input:focus{border-color:rgba(79,70,229,.4);background:#faf9ff}
        .part-row .stock-info{font-size:.68rem;color:var(--text-s);white-space:nowrap}
        .part-row .total-cell{font-size:.8rem;font-weight:700;color:var(--primary-2);min-width:80px;text-align:right}
        .btn-rm{width:28px;height:28px;border-radius:7px;background:#fee2e2;border:none;
            color:var(--red);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:.7rem}
        .btn-rm:hover{background:#fca5a5}

        /* WARRANTY NOTICE */
        .warranty-notice{background:linear-gradient(135deg,#d1fae5,#ecfdf5);
            border:1.5px solid #a7f3d0;border-radius:12px;padding:14px 16px;
            display:flex;align-items:center;gap:10px;margin-bottom:14px}
        .warranty-notice i{color:var(--green);font-size:1.1rem;flex-shrink:0}
        .warranty-notice-text{font-size:.8rem;color:#065f46;line-height:1.5}
        .warranty-notice-text strong{font-weight:700}

        /* COST SUMMARY */
        .cost-summary{background:#fafbff;border:1px solid var(--border-light);border-radius:12px;padding:14px 16px}
        .cost-row{display:flex;justify-content:space-between;align-items:center;
            padding:5px 0;font-size:.82rem;color:var(--text-m)}
        .cost-row.total{border-top:2px solid var(--border-light);margin-top:8px;padding-top:10px;
            font-size:.92rem;font-weight:700;color:var(--text-h)}
        .cost-row.free{color:var(--green)}
        .cost-val{font-weight:600;color:var(--text-h)}
        ...
        .btn-warning{background:var(--amber);color:#fff;box-shadow:0 3px 10px rgba(217,119,6,.28)}
        ...
        .co-tech-item{display:flex;align-items:center;justify-content:space-between;
            padding:8px 12px;border-radius:10px;background:#fafbff;
            border:1px solid var(--border-light);margin-bottom:6px;font-size:.8rem}
        .co-tech-name{font-weight:600;color:var(--text-h)}
        .co-tech-status{font-size:.68rem}
    </style>
</head>
<body>
...
function addPartRow() {
    ...
    sel.name = 'partTypeId';
    sel.onchange = function() { updatePartRow(this); };
    ...
    qtyInput.onchange = function() { updateRowTotal(this); };
    ...
    rmBtn.onclick = function() { removePartRow(this); };
...
function recalcTotal() {
    ...
    if (!isWarranty && el('partsSubtotal')) el('partsSubtotal').textContent = formatVND(partsTotal);
    if (el('laborDisplay')) el('laborDisplay').textContent = formatVND(labor);
    if (el('vatDisplay'))   el('vatDisplay').textContent   = formatVND(vat);
    if (el('grandTotal'))   el('grandTotal').textContent   = formatVND(grand);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (lc) lc.addEventListener('input', recalcTotal);
    recalcTotal();
});
