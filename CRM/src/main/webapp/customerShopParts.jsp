...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .shop-tab:hover{background:var(--primary-light); color:var(--primary-2)}
        .shop-tab.active{
        ...
        .search-input::placeholder{color:var(--text-s)}
        .search-input:focus{
        ...
        .btn-search:hover{background:#4338ca;transform:translateY(-1px);box-shadow:0 6px 20px rgba(79,70,229,0.42)}

        .result-info{font-size:.78rem; color:var(--text-s); margin-bottom:18px}
        .result-info strong{color:var(--text-b)}
        ...
        .product-name{
            ...
            color:var(--text-h); margin-bottom:5px; line-height:1.3;
        }
        .product-desc{
            font-size:.76rem; color:var(--text-m);
            margin-bottom:11px; flex:1; line-height:1.5;
            overflow:hidden;
        ...
        /* Add to cart form */
        .add-to-cart-form{display:flex; gap:7px}
        .qty-input{
            width:54px; padding:8px;
            background:#fff;
            border:1.5px solid var(--border-light);
            border-radius:8px;
            font-family:'Sora',sans-serif;
            font-size:.81rem; text-align:center;
            color:var(--text-b); outline:none;
            transition:border-color .2s;
        }
        .qty-input:focus{border-color:rgba(79,70,229,0.4)}
        .btn-add{
            flex:1; padding:8px;
        ...
        .btn-add:hover{background:#15803d;transform:translateY(-1px);box-shadow:0 6px 16px rgba(22,163,74,0.35)}
        ...
        .page-btn:hover{background:var(--primary-light);border-color:rgba(99,102,241,0.3);color:var(--primary-2)}
        .page-btn.active{
            background:var(--primary);
        ...
        .empty-state i{font-size:2.8rem; display:block; margin-bottom:14px; opacity:.2; color:var(--text-m)}
    </style>
</head>
<body>
    ...
    <aside class="sb">
        ...
        <nav class="sb-nav">
            ...
            <a href="<%=ctx%>/customerShop?action=parts" class="sb-item on">
    ...
    <main class="main">
        ...
        <div class="topbar">
            <div>
                <div class="topbar-title">
                    <i class="fas fa-store" style="color:var(--teal);margin-right:8px;font-size:1rem"></i>
                    Shop — Parts
                </div>
                <div class="topbar-sub">Browse and purchase spare parts for your equipment</div>
            </div>
        ...
        <div class="content">
            ...
            <%-- Shop Tabs --%>
            <div class="shop-tabs">
                <a href="<%=ctx%>/customerShop?action=parts" class="shop-tab active">
                    <i class="fas fa-puzzle-piece"></i> Parts
                </a>
                <a href="<%=ctx%>/customerShop?action=equipment" class="shop-tab">
                    <i class="fas fa-server"></i> Equipment
                </a>
            </div>
            ...
            <form method="get" action="<%=ctx%>/customerShop">
                ...
                <div class="toolbar">
                    <input class="search-input" type="text" name="keyword"
                           placeholder="Search parts..." value="<%=keyword%>">
            ...
            <div class="result-info">Found <strong><%=total%></strong> part(s)</div>
            ...
            <div class="product-grid">
                ...
                <div class="product-card">
                    ...
                    <div class="product-body">
                        ...
                        <a href="<%=ctx%>/customerShop?action=detail&itemType=PART&id=<%=item.id%>"
                           style="text-decoration:none">
                        <div class="product-name" style="cursor:pointer;transition:color .2s"
                        ...
                        <div class="product-stock <%=item.availableQty<=3?"low":""%>">
                            <i class="fas fa-<%=item.availableQty<=3?"exclamation-triangle":"check-circle"%>"></i>
                            <%=item.availableQty<=3?"Low stock — ":""%><%=item.availableQty%> units available
                        </div>
                        <form method="post" action="<%=ctx%>/customerShop" class="add-to-cart-form">
                            ...
                            <input class="qty-input" type="number" name="quantity"
                                   value="1" min="1" max="<%=item.availableQty%>">
            ...
            <div class="pagination">
                <a href="?action=parts&page=1&keyword=<%=keyword%>&categoryId=<%=catId%>&sortBy=<%=sortBy%>"
                   class="page-btn <%=currentPage==1?"disabled":""%>">«</a>
                <a href="?action=parts&page=<%=Math.max(1,currentPage-1)%>&keyword=<%=keyword%>&categoryId=<%=catId%>&sortBy=<%=sortBy%>"
                   class="page-btn <%=currentPage==1?"disabled":""%>">‹</a>
                <%for(int p=Math.max(1,currentPage-2);p<=Math.min(totalPages,currentPage+2);p++){%>
                <a href="?action=parts&page=<%=p%>&keyword=<%=keyword%>&categoryId=<%=catId%>&sortBy=<%=sortBy%>"
                   class="page-btn <%=p==currentPage?"active":""%>"><%=p%></a>
                <%}%>
                <a href="?action=parts&page=<%=Math.min(totalPages,currentPage+1)%>&keyword=<%=keyword%>&categoryId=<%=catId%>&sortBy=<%=sortBy%>"
                   class="page-btn <%=currentPage==totalPages?"disabled":""%>">›</a>
                <a href="?action=parts&page=<%=totalPages%>&keyword=<%=keyword%>&categoryId=<%=catId%>&sortBy=<%=sortBy%>"
                   class="page-btn <%=currentPage==totalPages?"disabled":""%>">»</a>
            </div>
            <%}%>
