...
<%
    ...
    List<WorkTask> tasks = (List<WorkTask>) request.getAttribute("tasks");
...
<head>
    ...
    <style>
        :root {
            --sb-bg:#1e1b4b; --sb-border:rgba(255,255,255,0.08);
            --sb-text:rgba(255,255,255,0.45); --sb-accent:#818cf8;
            --sb-accent-2:#a5b4fc; --sb-item-on:rgba(129,140,248,0.2);
            --sb-width:252px;
            --bg:#f3f4f9; --bg-card:#ffffff; --bg-topbar:#ffffff;
            --border-light:#e8ecf5; --border-light2:#f0f2fb;
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        /* SIDEBAR */
        .sb{width:var(--sb-width);min-height:100vh;background:var(--sb-bg);border-right:1px solid rgba(79,70,229,0.2);
            display:flex;flex-direction:column;position:fixed;top:0;left:0;z-index:100;box-shadow:4px 0 24px rgba(0,0,0,0.15)}
        ...
        .sb-role{display:inline-flex;align-items:center;background:rgba(13,148,136,0.2);
            border:1px solid rgba(13,148,136,0.35);color:#5eead4;font-size:.6rem;font-weight:700;
        ...
        .sb-item i{width:28px;height:28px;display:flex;align-items:center;justify-content:center;
            font-size:.78rem;border-radius:8px;background:rgba(255,255,255,0.06);flex-shrink:0}
        ...
        .sb-ava{width:34px;height:34px;border-radius:50%;background:linear-gradient(135deg,#818cf8,#a78bfa);
            display:flex;align-items:center;justify-content:center;color:#fff;font-size:.88rem;font-weight:700;
        ...
        .sc{border-radius:16px;padding:18px 16px;position:relative;overflow:hidden;color:#fff;
        ...
        .sc-assigned{background:var(--amber);   box-shadow:0 4px 20px rgba(217,119,6,0.3)}
        ...
        .sc-cancel  {background:#6b7280;        box-shadow:0 4px 20px rgba(107,114,128,0.3)}
        .sc::after{content:'';position:absolute;width:80px;height:80px;border-radius:50%;
            background:rgba(255,255,255,0.12);top:-20px;right:-20px}
        .sc-icon{width:34px;height:34px;border-radius:9px;display:flex;align-items:center;
            justify-content:center;font-size:.82rem;margin-bottom:10px;
            background:rgba(255,255,255,0.2);position:relative;z-index:1}
        ...
        /* FILTER BAR */
        .filter-bar{background:var(--bg-card);border:1px solid var(--border-light);border-radius:14px;
            padding:14px 16px;display:flex;flex-wrap:wrap;gap:10px;align-items:center;
            margin-bottom:18px;box-shadow:0 1px 4px rgba(0,0,0,0.04)}
        .filter-bar select{padding:9px 13px;border:1.5px solid var(--border-light);border-radius:9px;
            font-size:.81rem;font-family:'Sora',sans-serif;color:var(--text-b);background:#fff;
            outline:none;transition:all .2s}
        .filter-bar select:focus{border-color:rgba(79,70,229,0.4);background:#faf9ff}
        .btn{display:inline-flex;align-items:center;gap:7px;padding:9px 18px;border-radius:10px;
            font-size:.81rem;font-weight:600;font-family:'Sora',sans-serif;cursor:pointer;
        ...
        /* TABLE */
        .table-wrap{background:var(--bg-card);border:1px solid var(--border-light);
            border-radius:16px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,0.05)}
        ...
        .pagination{display:flex;justify-content:flex-end;align-items:center;gap:5px;
            padding:13px 16px;border-top:1px solid var(--border-light2)}
        .pagination a,.pagination span{padding:6px 12px;border-radius:8px;font-size:.77rem;
            font-weight:500;text-decoration:none;color:var(--text-m);border:1.5px solid var(--border-light);
            background:#fff;transition:all .15s}
        .pagination a:hover{background:var(--primary-light);border-color:rgba(99,102,241,0.3);color:var(--primary-2)}
        .pagination .active{background:var(--primary);color:#fff;border-color:transparent}
        .pagination .dots{border:none;background:none;color:var(--text-s)}
        .section-lbl{font-size:.63rem;font-weight:700;text-transform:uppercase;letter-spacing:2px;
            color:var(--primary-2);margin-bottom:13px;display:flex;align-items:center;gap:10px}
        ...
        .td-empty i{font-size:2rem;display:block;margin-bottom:10px;opacity:.2}
...
<aside class="sb">
    <div class="sb-brand">
        ...
        <div>
            ...
            <div class="sb-role">Technician</div>
        </div>
    </div>
    <nav class="sb-nav">
...
<main class="main">
    <div class="topbar">
        <div>
            <div class="topbar-title"><i class="fas fa-tasks" style="color:var(--primary-2);margin-right:8px;font-size:1rem"></i>My Tasks</div>
            <div class="topbar-sub">View and manage your assigned service tasks</div>
        </div>
    </div>
    ...
    <div class="content">
        ...
        <div class="stats-grid">
            ...
            <div class="sc sc-assigned">
                <div class="sc-icon"><i class="fas fa-inbox"></i></div>
                <div class="sc-val"><%=stats.getOrDefault("assigned",0)%></div>
                <div class="sc-lbl">Assigned</div>
            </div>
            ...
            <div class="sc sc-cancel">
                <div class="sc-icon"><i class="fas fa-ban"></i></div>
                <div class="sc-val"><%=stats.getOrDefault("cancelled",0)%></div>
                <div class="sc-lbl">Cancelled</div>
            </div>
        </div>

        <div class="section-lbl">Filter</div>
        <form method="get" action="<%=ctx%>/techTasks">
        <div class="filter-bar">
            <select name="status">
                <option value="">All Status</option>
                <%for(String s : new String[]{"Assigned","In Progress","Completed","Cancelled"}){%>
                <option value="<%=s%>" <%=s.equals(fStatus)?"selected":""%>><%=s%></option>
                <%}%>
            </select>
            <button type="submit" class="btn btn-primary"><i class="fas fa-search"></i> Filter</button>
            <a href="<%=ctx%>/techTasks" class="btn btn-secondary"><i class="fas fa-times"></i> Reset</a>
        </div>
        </form>
        ...
        <div class="table-wrap">
            <table>
                ...
                <%}else{ int idx=1; for(WorkTask t : tasks){
                    String tsCls = "b-assigned";
                    if("In Progress".equals(t.getStatus())) tsCls="b-progress";
                    else if("Completed".equals(t.getStatus())) tsCls="b-completed";
                    else if("Cancelled".equals(t.getStatus())) tsCls="b-cancelled";
                %>
                <tr>
                    <td class="td-muted"><%=idx++%></td>
                    <td style="font-family:'Courier New',monospace;font-size:.78rem;font-weight:700;color:var(--primary-2)">
                        <%=t.getRequestId()!=null?"SR#"+t.getRequestId():"—"%>
                    </td>
                    <%-- Title --%>
<td style="color:var(--text-b);font-weight:500">
    <%=t.getSrTitle()!=null ? t.getSrTitle() : (t.getTaskDetails()!=null?t.getTaskDetails():"—")%>
</td>

<%-- Contract Type --%>
<td>
    <%if(t.getContractType() != null){
        boolean isWr = "WARRANTY".equals(t.getContractType());%>
    <span class="ct-badge <%=isWr?"ct-wr":"ct-mt"%>"><%=isWr?"WARRANTY":"MAINTENANCE"%></span>
    <%}else{%>—<%}%>
</td>

<%-- Priority --%>
<td>
    <%if(t.getSrPriority() != null){
        String bPr="b-medium";
        if("HIGH".equals(t.getSrPriority()))    bPr="b-high";
        else if("URGENT".equals(t.getSrPriority())) bPr="b-urgent";
        else if("LOW".equals(t.getSrPriority()))    bPr="b-low";%>
    <span class="b <%=bPr%>"><%=t.getSrPriority()%></span>
    <%}else{%>—<%}%>
</td>
                    <td><span class="b <%=tsCls%>"><%=t.getStatus()%></span></td>
                    <td class="td-muted"><%=t.getCreatedAt()!=null?t.getCreatedAt().toString().replace("T"," ").substring(0,16):"—"%></td>
                    <td>
                        <a class="btn btn-primary btn-sm" href="<%=ctx%>/techTasks?action=detail&id=<%=t.getId()%>">
            ...
            <%if(totalPages>1){
                String q = "&status=" + (fStatus!=null?fStatus:"");
            %>
            <div class="pagination">
                <%if(currentPage>1){%>
                <a href="<%=ctx%>/techTasks?page=<%=currentPage-1%><%=q%>"><i class="fas fa-chevron-left"></i></a>
                <%}%>
                <%for(int i=1;i<=totalPages;i++){
                    if(i==currentPage){%><span class="active"><%=i%></span>
                    <%}else if(i==1||i==totalPages||Math.abs(i-currentPage)<=2){%>
                    <a href="<%=ctx%>/techTasks?page=<%=i%><%=q%>"><%=i%></a>
                    <%}else if(Math.abs(i-currentPage)==3){%><span class="dots">…</span><%}%>
                <%}%>
                <%if(currentPage<totalPages){%>
                <a href="<%=ctx%>/techTasks?page=<%=currentPage+1%><%=q%>"><i class="fas fa-chevron-right"></i></a>
                <%}%>
            </div>
            <%}%>
        </div>
