...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .badge-eq  {background:#e0f2fe;border:1px solid rgba(2,132,199,0.3);color:var(--info)}
        ...
        .cat-badge{display:inline-flex;align-items:center;gap:6px;padding:4px 12px;border-radius:20px;font-size:.68rem;font-weight:700;letter-spacing:.8px;text-transform:uppercase;margin-bottom:10px;background:#e0f2fe;border:1px solid rgba(2,132,199,0.25);color:var(--info);}
    ...
    <div class="content">
        ...
        <div class="bottom-grid">
            ...
            <div class="detail-card">
                ...
                <div class="spec-row"><span class="spec-key">Retired</span><span class="spec-val" style="color:var(--text-s)"><%=et.getRetiredUnits()%></span></div>
            ...
            <div class="detail-card" style="grid-column:span 2">
                <div class="detail-card-hd">
                    <div class="dc-icon" style="background:#e0f2fe;color:var(--info)"><i class="fas fa-boxes-stacked"></i></div>
                ...
                <div class="units-table-wrap">
                    <table>
                        <thead>
                            <tr><th>Unit ID</th><th>Serial Number</th><th>Status</th><th>Created</th><th>Updated</th></tr>
                        </thead>
                        <tbody>
                        <%for(EquipmentUnit eu:units){
                            String st=eu.getStatus().toUpperCase();
                            String badgeClass="AVAILABLE".equals(st)?"s-available":("INUSE".equals(st)||"IN_USE".equals(st))?"s-inuse":"FAULTY".equals(st)?"s-faulty":"s-retired";
                            String iconClass ="AVAILABLE".equals(st)?"circle-check":("INUSE".equals(st)||"IN_USE".equals(st))?"screwdriver-wrench":"FAULTY".equals(st)?"triangle-exclamation":"archive";
                        %>
                        <tr>
                            <td style="font-family:'Courier New',monospace;color:var(--primary-2);font-weight:600">#<%=eu.getId()%></td>
                            ...
                            <td style="color:var(--text-s);font-size:.74rem"><%=eu.getCreatedAt()!=null?eu.getCreatedAt().toLocalDate():"—"%></td>
                            <td style="color:var(--text-s);font-size:.74rem"><%=eu.getUpdatedAt()!=null?eu.getUpdatedAt().toLocalDate():"—"%></td>
                        </tr>
                        <%}%>
                        </tbody>
                    </table>
...
<div class="modal-overlay" id="deleteModal">
    <div class="modal">
        ...
        <span style="color:var(--red);font-size:.78rem">This will also delete all <%=et.getTotalUnits()%> units. This cannot be undone.</span></p>
        <form method="post" action="<%=ctx%>/numberEquipment">
