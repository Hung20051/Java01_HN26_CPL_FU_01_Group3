...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sb-role{display:inline-flex;background:rgba(129,140,248,0.2);border:1px solid rgba(129,140,248,0.3);color:var(--sb-accent-2);font-size:.6rem;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:2px 8px;border-radius:20px;margin-top:3px;}
        ...
        .topbar-sub{font-size:.75rem;color:var(--text-s);margin-top:1px}
        ...
        /* search bar */
        .search-bar{background:#fff;border:1px solid var(--border-light);border-radius:14px;padding:14px 18px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;margin-bottom:20px;}
        ...
        .search-bar input:focus{border-color:var(--primary-2)}
        /* grid cards */
        @keyframes cardIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}
        .eq-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;}
        .eq-card{background:#fff;border:1px solid var(--border-light);border-radius:16px;overflow:hidden;transition:all .2s;text-decoration:none;color:inherit;display:block;animation:cardIn .4s ease both;}
        .eq-card:hover{transform:translateY(-3px);box-shadow:0 8px 28px rgba(79,70,229,0.12);border-color:rgba(99,102,241,0.3);}
        .eq-img{height:140px;background:linear-gradient(135deg,#f0f4ff,#e8ecf5);display:flex;align-items:center;justify-content:center;overflow:hidden;position:relative;}
        .eq-img img{width:100%;height:100%;object-fit:cover}
        .eq-img-placeholder{font-size:3rem;color:var(--primary-2);opacity:.25}
        .eq-body{padding:16px}
        .eq-model{font-size:.92rem;font-weight:700;color:var(--text-h);margin-bottom:2px}
        .eq-cat{font-size:.73rem;color:var(--text-s);margin-bottom:12px}
        .eq-stats{display:flex;gap:6px;flex-wrap:wrap;margin-bottom:12px;}
        .eq-stat{font-size:.69rem;padding:3px 9px;border-radius:20px;font-weight:700;}
        .eq-stat.avail{background:#d1fae5;color:#065f46}
        .eq-stat.inuse{background:#dbeafe;color:#1e40af}
        .eq-stat.faulty{background:#fee2e2;color:#991b1b}
        .eq-stat.total{background:#f3f4f6;color:#374151}
        .eq-price{font-size:.82rem;color:var(--primary-2);font-weight:700}
        /* summary bar */
        .summary-bar{background:linear-gradient(135deg,#1e1b4b,#3730a3);border-radius:14px;padding:16px 22px;display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;color:#fff;}
        .summary-bar-num{font-size:1.8rem;font-weight:800;letter-spacing:-1px}
        .summary-bar-lbl{font-size:.78rem;opacity:.7;margin-top:2px}
        /* pager */
        .pager{display:flex;align-items:center;justify-content:center;gap:6px;padding:24px 0 0;}
        ...
        .pager a:hover{border-color:var(--primary-2);color:var(--primary-2)}
        .pager .active{background:var(--primary);color:#fff;border-color:var(--primary);}
        .empty{text-align:center;padding:60px 24px;color:var(--text-s);}
        .empty i{font-size:3rem;display:block;margin-bottom:16px;opacity:.15}
    </style>
</head>
<body>
<!-- SIDEBAR -->
...
<main class="main">
    <div class="topbar">
        <div>
            ...
            <div class="topbar-sub">Danh sách thiết bị trong hệ thống — chỉ xem</div>
        </div>
        <div class="topbar-right">
            <a href="<%=ctx%>/technicianContracts" class="btn btn-outline btn-sm"><i class="fas fa-file-contract"></i> Contracts</a>
        </div>
    </div>
    ...
    <div class="content">
        ...
        <div class="summary-bar">
            <div>
                <div class="summary-bar-num"><%=total%></div>
                <div class="summary-bar-lbl">Loại thiết bị</div>
            </div>
            <i class="fas fa-desktop" style="font-size:2.5rem;opacity:.25"></i>
        </div>
        ...
        <form method="get" action="<%=ctx%>/technicianContracts">
            ...
            <div class="search-bar">
                <input type="text" name="keyword" value="<%=keyword%>" placeholder="Tìm theo model, mô tả, danh mục...">
        ...
        <div class="empty">
            ...
            <small style="font-size:.75rem">Thử tìm với từ khóa khác</small>
        </div>
        <%}else{%>
        <div class="eq-grid">
            ...
            <a href="<%=ctx%>/technicianContracts?action=equipmentDetail&id=<%=et.getId()%>" class="eq-card">
                <div class="eq-img">
                    ...
                    <%}else{%>
                    <div class="eq-img-placeholder"><i class="fas fa-desktop"></i></div>
                    <%}%>
                </div>
                <div class="eq-body">
                    <div class="eq-model"><%=et.getModel()%></div>
                    <div class="eq-cat"><%=et.getCategoryName()!=null?et.getCategoryName():""%></div>
                    <div class="eq-stats">
                        <span class="eq-stat avail"><i class="fas fa-check-circle" style="margin-right:3px"></i><%=et.getAvailableUnits()%> sẵn</span>
                        <span class="eq-stat inuse"><i class="fas fa-tools" style="margin-right:3px"></i><%=et.getInuseUnits()%> đang dùng</span>
                        <%if(et.getFaultyUnits()>0){%><span class="eq-stat faulty"><i class="fas fa-exclamation-circle" style="margin-right:3px"></i><%=et.getFaultyUnits()%> lỗi</span><%}%>
                        <span class="eq-stat total">Tổng: <%=et.getTotalUnits()%></span>
                    </div>
                    <div class="eq-price"><%=nf.format((long)et.getUnitPrice())%> ₫</div>
                </div>
            </a>
            <%}%>
        </div>
        ...
        <%if(totalPages>1){%>
        <div class="pager">
            <%String kw_enc=java.net.URLEncoder.encode(keyword,"UTF-8");%>
            <%if(page>1){%><a href="?action=equipment&page=<%=page-1%>&keyword=<%=kw_enc%>"><i class="fas fa-chevron-left"></i></a><%}%>
            <%for(int p2=Math.max(1,page-2);p2<=Math.min(totalPages,page+2);p2++){%>
            <<%=p2==page?"span class=\"active\"":"a href=\"?action=equipment&page="+p2+"&keyword="+kw_enc+"\""%>><%=p2%></<%=p2==page?"span":"a"%>>
            <%}%>
            <%if(page<totalPages){%><a href="?action=equipment&page=<%=page+1%>&keyword=<%=kw_enc%>"><i class="fas fa-chevron-right"></i></a><%}%>
        </div>
        <%}%>
        <%}%>
    </div>
