...
<html lang="en">
        ...
        <style>
            ...
            *,*::before,*::after{
            ...
            .online-dot{
                ...
                background:#4a5568;
                border-radius:50%;
                border:2px solid rgba(11,20,55,0.85);
            ...
            .act-btn.pin-act:hover{
                background:rgba(251,191,36,0.2);
                border-color:rgba(251,191,36,0.4);
                color:var(--amber);
                transform:scale(1.08);
            }
            .act-btn.pinned-active{
            ...
            .msg-row.mine .react-popup{
                right:0;
            }
            .msg-row.other .react-popup{
                left:0;
            }
            @keyframes popIn{
                from{
            ...
            .upload-progress.active{
                animation:uploadAnim 1.2s ease infinite alternate;
            }
            ...
            .emoji-burst-particle{
                ...
                z-index:9999;
                user-select:none;
                animation:emojiBurst var(--dur,.9s) ease-out forwards;
            }
            @keyframes emojiBurst{
                ...
                15%{
                    opacity:1;
                    transform:translate(calc(var(--tx)*.15),calc(var(--ty)*.15)) scale(1.5);
                }
                60%{
                    opacity:.85;
                }
                100%{
                    opacity:0;
                    transform:translate(var(--tx),var(--ty)) scale(.15) rotate(var(--rot));
                }
            }
        </style>
    </head>
    ...
    document.querySelectorAll('#chatMsgs .msg-row[data-id]').forEach(el => {
        ...
        if (mid) {
            ...
            el.querySelectorAll('.reaction-chip[data-emoji]').forEach(chip => {
                const emoji = chip.dataset.emoji, count = parseInt(chip.querySelector('.cnt').textContent) || 0, mine = chip.classList.contains('mine');
                if (emoji && count > 0)
                    reactions[mid][emoji] = {count, mine};
            });
        }
    });
    ...
    const scrollDown = s => {
        ...
        if (el)
            el.scrollTo({top: el.scrollHeight, behavior: s ? 'smooth' : 'instant'});
    };
    const isAtBottom = () => {
        const el = document.getElementById('chatMsgs');
        if (!el)
    ...
    function setAgentOnline(online) {
        ...
        if (txt) {
            txt.textContent = online ? 'Online · Support Agent' : 'Offline';
    ...
    async function uploadFile(file) {
        ...
        fd.append('file', file);
        try {
            const resp = await fetch(CTX + '/customerChat', {method: 'POST', body: fd});
    ...
    const buildAvatarHtml = isMine => {
        const url = isMine ? MY_AVATAR : AGENT_AVATAR, letter = (isMine ? MY_NAME : AGENT_NAME).substring(0, 1).toUpperCase();
        return url ? '<img src="' + url + '" alt="avatar">' : letter;
    };
// customerChat.jsp và supportChat.jsp — thêm CTX check:
    ...
    const buildStatusHtml = (m, isMine) => {
        if (!isMine || m.recalled)
            return '';
        const isRead = m.read === true, isDelivered = m.delivered === true;
    ...
    function buildMsgRow(m, isMine) {
        ...
        if (m.reactions && Array.isArray(m.reactions))
            m.reactions.forEach(rx => {
                if (rx.count > 0)
                    rxHtml += '<div class="reaction-chip ' + (rx.mine ? 'mine' : '') + '" data-emoji="' + rx.emoji + '" onclick="toggleReactionChip(this,\'' + rx.emoji + '\',' + m.id + ')">' + rx.emoji + ' <span class="cnt">' + rx.count + '</span></div>';
            });
        rxHtml += '</div>';
    ...
    function pollUpdates() {
        fetch(CTX + '/customerChat?action=pollUpdates').then(r => r.json()).then(allMsgs => {
            ...
            allMsgs.forEach(m => {
                ...
                const row = document.querySelector('#chatMsgs .msg-row[data-id="' + mid + '"]');
                if (!row)
                    return;
                ...
                if (m.recalled) {
                    ...
                    if (b && !b.classList.contains('recalled')) {
                        b.classList.add('recalled');
                        b.innerHTML = '<i class="fas fa-rotate-left" style="margin-right:5px;font-size:.7rem"></i>Message recalled';
                ...
                if (m.reactions && Array.isArray(m.reactions)) {
                    ...
                    m.reactions.forEach(rx => {
                        if (rx.count > 0)
                            reactions[mid][rx.emoji] = {count: rx.count, mine: rx.mine};
                    });
                ...
                if (m.pinned) {
                    if (String(pinnedMsgId) !== mid) {
                        if (pinnedMsgEl) {
                            const ob = pinnedMsgEl.querySelector('.pin-act');
                            if (ob)
                                ob.classList.remove('pinned-active');
                            pinnedMsgEl.removeAttribute('data-pinned');
                        }
                        pinnedMsgId = mid;
                        pinnedMsgEl = row;
                        row.dataset.pinned = 'true';
                        const pb2 = row.querySelector('.pin-act');
                        if (pb2)
                            pb2.classList.add('pinned-active');
                        if (pt) {
                            const b = row.querySelector('.msg-bubble');
                            let txt = '';
                            if (b) {
                    ...
                    if (pinnedMsgEl === row) {
                        ...
                        const pb2 = row.querySelector('.pin-act');
                        if (pb2)
                            pb2.classList.remove('pinned-active');
                        if (pb)
                ...
                // Tick
                if (isMine && !m.recalled)
                    updateMsgStatus(row, m.read, m.delivered);
            });
        }).catch(() => {
        });
    }
    function updateMsgStatus(row, isRead, isDelivered) {
        let s = row.querySelector('.msg-status');
        if (!s) {
            const tr = row.querySelector('.msg-time-row');
            if (!tr)
                return;
        ...
        const cls = isRead ? 'read' : (isDelivered ? 'delivered' : ''), icon = isRead ? 'fa-check-double' : (isDelivered ? 'fa-check-double' : 'fa-check');
        s.className = 'msg-status ' + cls;
        s.innerHTML = '<i class="fas ' + icon + '"></i>';
    }
    ...
    function recallMsg(btn) {
        ...
        if (!msgId || msgId.includes('temp')) {
            showToast('Cannot recall unsent message');
            return;
        }
    ...
    function pinMsg(btn) {
        ...
        if (!row)
            return;
        const b = row.querySelector('.msg-bubble');
        if (b.classList.contains('recalled')) {
            showToast('Cannot pin a recalled message');
            return;
        }
        const msgId = row.dataset.id;
        if (!msgId || msgId.includes('temp')) {
            showToast('Cannot pin unsent message');
            return;
        }
        ...
        if (isAlreadyPinned) {
            ...
            if (pb)
                pb.classList.add('show');
            b.style.outline = '2px solid var(--amber)';
    ...
    function scrollToPin() {
        ...
        const b = pinnedMsgEl.querySelector('.msg-bubble');
        if (b) {
            b.style.outline = '2px solid var(--amber)';
            setTimeout(() => b.style.outline = '', 1500);
        }
    ...
    function clearPin() {
        ...
        showToast('Pin removed');
        if (oldId)
            fetch(CTX + '/customerChat', {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}, body: new URLSearchParams({action: 'pin', messageId: oldId})}).catch(() => {
            });
    }
    ...
    function addReaction(btn, emoji) {
        const popup = btn.closest('.react-popup'), row = btn.closest('.msg-row'), msgId = row?.dataset.id;
        if (!msgId || msgId.includes('temp'))
            return;
        triggerEmojiBurst(emoji, btn);
        popup.classList.remove('show');
        _toggleReactionLocal(row, msgId, emoji);
        fetch(CTX + '/customerChat', {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}, body: new URLSearchParams({action: 'react', messageId: msgId, emoji})}).catch(() => {
        });
    }
    function toggleReactionChip(chip, emoji, msgId) {
        ...
        fetch(CTX + '/customerChat', {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}, body: new URLSearchParams({action: 'react', messageId: mid, emoji})}).catch(() => {
        });
    }
    function _toggleReactionLocal(row, mid, emoji) {
        if (!reactions[mid])
            reactions[mid] = {};
        if (!reactions[mid][emoji])
            reactions[mid][emoji] = {count: 0, mine: false};
        const r = reactions[mid][emoji];
        r.mine = !r.mine;
        r.count += r.mine ? 1 : -1;
        if (r.count <= 0)
            delete reactions[mid][emoji];
        renderReactions(row, mid);
    }
    function renderReactions(row, msgId) {
        ...
        if (!c)
            return;
        c.innerHTML = '';
        Object.entries(reactions[msgId] || {}).forEach(([emoji, r]) => {
            if (r.count <= 0)
                return;
    ...
    function triggerEmojiBurst(emoji, el) {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            span.className = 'emoji-burst-particle';
            span.textContent = emoji;
            const angle = Math.random() * 360 * (Math.PI / 180), dist = 120 * (.4 + Math.random() * .6);
            span.style.cssText = 'left:' + cx + 'px;top:' + cy + 'px;margin:-12px 0 0 -12px;--tx:' + (Math.cos(angle) * dist) + 'px;--ty:' + (Math.sin(angle) * dist - 25) + 'px;--rot:' + ((Math.random() - .5) * 420) + 'deg;--dur:' + (.55 + Math.random() * .55) + 's;animation-delay:' + (Math.random() * .12) + 's';
    ...
    document.addEventListener('click', e => {
        const p = document.getElementById('emojiPicker'), t = document.getElementById('emojiToggleBtn');
        if (p && t && !p.contains(e.target) && e.target !== t)
            p.classList.remove('show');
    });

    const hideTimers = new WeakMap();
    function showActions(row) {
        if (hideTimers.has(row)) {
            clearTimeout(hideTimers.get(row));
    ...
    function bindRowHover(row) {
        ...
        const a = row.querySelector('.msg-actions');
        if (a) {
            a.addEventListener('mouseenter', () => showActions(row));
            a.addEventListener('mouseleave', () => scheduleHideActions(row));
        }
    }
    ...
    window.addEventListener('beforeunload', () => {
        ...
        if (isTypingSent)
            navigator.sendBeacon(CTX + '/customerChat', new URLSearchParams({action: 'stopTyping'}));
    });
        </script>
