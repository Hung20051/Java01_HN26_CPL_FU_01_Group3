...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        html{height:100%}
        body{font-family:'DM Sans',sans-serif;background:var(--bg-root);color:var(--text);min-height:100vh;overflow-x:hidden}
        ::-webkit-scrollbar{width:4px}
        ::-webkit-scrollbar-thumb{background:rgba(226,185,90,.35);border-radius:4px}
        ...
        .sb-back{display:inline-flex;align-items:center;gap:8px;color:var(--muted);text-decoration:none;font-size:.76rem;font-weight:500;letter-spacing:.6px;text-transform:uppercase;padding:6px 0;margin-bottom:32px;transition:color .2s;border:none;background:none;cursor:pointer;font-family:inherit}
        .sb-back:hover{color:var(--text-2)}
        .sb-back i{font-size:.7rem}

        /* Avatar */
        .sb-avatar-wrap{position:relative;width:96px;height:96px;margin:0 auto 18px;cursor:pointer}
        ...
        .sb-avatar img{width:96px;height:96px;border-radius:50%;object-fit:cover}
        /* Spinning ring */
        .sb-avatar::before{
        ...
        .sb-avatar-overlay{position:absolute;inset:0;border-radius:50%;background:rgba(13,14,45,.7);display:flex;align-items:center;justify-content:center;color:var(--gold);font-size:1.3rem;opacity:0;transition:.25s;backdrop-filter:blur(4px)}

        .sb-name{text-align:center;font-family:'Cormorant Garamond',serif;font-size:1.4rem;font-weight:600;color:var(--text);line-height:1.25;margin-bottom:6px}
        .sb-role{display:flex;align-items:center;justify-content:center;gap:6px;font-size:.68rem;font-weight:500;letter-spacing:1.8px;text-transform:uppercase;color:var(--gold);margin-bottom:20px}
        .sb-status{display:flex;align-items:center;justify-content:center;gap:6px;font-size:.74rem;color:var(--text-2);margin-bottom:24px}
        .sb-dot{width:7px;height:7px;border-radius:50%;background:var(--success);box-shadow:0 0 10px var(--success);animation:dotPulse 2.5s ease-in-out infinite}
        @keyframes dotPulse{0%,100%{box-shadow:0 0 6px var(--success)}50%{box-shadow:0 0 18px var(--success),0 0 30px rgba(94,217,176,.3)}}

        .sb-divider{border:none;border-top:1px solid var(--rim);margin:0 0 20px}
        ...
        .sb-tab i{width:15px;text-align:center;font-size:.78rem;flex-shrink:0}
        .sb-tab:hover{color:var(--text-2);background:rgba(255,255,255,.06);border-color:var(--rim)}
        .sb-tab.active{
            color:var(--gold);
            background:linear-gradient(90deg, rgba(226,185,90,.12), rgba(226,185,90,.04));
            border-color:rgba(226,185,90,.3);
        }
        .sb-tab.active i{filter:drop-shadow(0 0 6px var(--gold-glow))}
        ...
        .sb-username i{color:var(--muted);font-size:.72rem}

        /* ══════════════ MAIN ══════════════ */
        .main{flex:1;padding:44px 48px 80px;min-width:0}

        .flash{display:flex;align-items:center;gap:10px;padding:12px 16px;border-radius:12px;font-size:.83rem;font-weight:500;margin-bottom:26px;animation:slideDown .3s ease}
        @keyframes slideDown{from{opacity:0;transform:translateY(-8px)}to{opacity:1;transform:none}}
        .flash-ok {background:rgba(94,217,176,.1);color:var(--success);border:1px solid rgba(94,217,176,.25)}
        .flash-err{background:rgba(244,123,116,.1);color:var(--danger);border:1px solid rgba(244,123,116,.25)}

        .section-eyebrow{font-size:.63rem;letter-spacing:2.8px;text-transform:uppercase;color:var(--gold);font-weight:600;margin-bottom:7px;display:flex;align-items:center;gap:10px}
        .section-eyebrow::after{content:'';flex:1;height:1px;background:linear-gradient(to right,rgba(226,185,90,.35),transparent)}
        .section-title{font-family:'Cormorant Garamond',serif;font-size:1.9rem;font-weight:300;letter-spacing:-.4px;color:var(--text);margin-bottom:28px;line-height:1.15}
        ...
        /* ── Completion bar ── */
        .completion-wrap{margin-bottom:28px;background:var(--bg-card);border:1px solid var(--rim);border-radius:14px;padding:18px 22px}
        .completion-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;font-size:.78rem;color:var(--text-2)}
        .completion-header strong{color:var(--gold);font-size:.92rem}
        .completion-track{height:6px;background:rgba(255,255,255,.08);border-radius:4px;overflow:hidden}
        .completion-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,var(--blue),var(--gold));transition:width .9s cubic-bezier(.4,0,.2,1)}
        .completion-tip{font-size:.72rem;color:var(--muted);margin-top:8px}
        .completion-tip a{color:var(--gold);text-decoration:none;font-weight:600}

        /* ── Address banner ── */
        .address-banner{
            background:linear-gradient(135deg,rgba(91,143,248,.1),rgba(226,185,90,.06));
            border:1px solid rgba(91,143,248,.25);border-radius:16px;
            padding:20px 24px;display:flex;align-items:flex-start;gap:16px;margin-bottom:24px;
        }
        .address-banner-icon{font-size:1.5rem;color:var(--blue-2);margin-top:2px;flex-shrink:0;filter:drop-shadow(0 0 8px rgba(91,143,248,.5))}
        .address-banner-label{font-size:.62rem;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted);font-weight:600;margin-bottom:5px}
        .address-banner-value{font-size:.94rem;color:var(--text);font-weight:500;line-height:1.55}
        .address-banner-value.empty{color:var(--muted);font-style:italic}
        .map-link{display:inline-flex;align-items:center;gap:6px;margin-top:10px;font-size:.74rem;color:var(--blue-2);text-decoration:none;font-weight:500;transition:all .2s;padding:5px 12px;background:rgba(91,143,248,.12);border:1px solid rgba(91,143,248,.25);border-radius:8px}
        .map-link:hover{background:rgba(91,143,248,.22);color:#fff}

        /* ── Info grid ── */
        .info-grid{
            display:grid;grid-template-columns:1fr 1fr;gap:1px;
            background:rgba(255,255,255,.07);
            border:1px solid var(--rim);border-radius:16px;overflow:hidden;margin-bottom:24px;
        }
        .info-cell{background:var(--bg-layer);padding:20px 22px;transition:background .2s}
        .info-cell:hover{background:rgba(226,185,90,.04)}
        .info-cell.full-width{grid-column:1/-1}
        .info-cell-label{font-size:.6rem;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted);font-weight:600;margin-bottom:7px;display:flex;align-items:center;gap:6px}
        .info-cell-label i{color:var(--gold);font-size:.58rem}
        .info-cell-value{font-size:.9rem;font-weight:500;color:var(--text)}
        ...
        /* ── Form card ── */
        .form-card{background:rgba(255,255,255,.05);border:1px solid var(--rim);border-radius:16px;overflow:hidden;backdrop-filter:blur(12px)}
        .form-section{padding:26px 30px;border-bottom:1px solid var(--rim)}
        ...
        .field label{display:block;font-size:.68rem;font-weight:600;letter-spacing:.6px;text-transform:uppercase;color:var(--text-2);margin-bottom:8px}
        ...
        .field select option{background:#131638}
        ...
        .field input[readonly]{color:var(--muted);opacity:.55;cursor:not-allowed}
        .field-hint{font-size:.71rem;color:var(--muted);margin-top:6px;display:flex;align-items:center;gap:5px}

        .pass-wrap{position:relative}
        .pass-wrap input{padding-right:44px}
        .pass-eye{position:absolute;right:13px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--muted);cursor:pointer;font-size:.84rem;transition:color .2s}
        .pass-eye:hover{color:var(--gold)}
        .strength-track{height:3px;border-radius:2px;margin-top:8px;background:rgba(255,255,255,.08);overflow:hidden}
        .strength-fill{height:100%;border-radius:2px;transition:all .35s;width:0}
        .strength-label{font-size:.7rem;margin-top:5px}

        /* ── Buttons ── */
        .btn{display:inline-flex;align-items:center;gap:8px;padding:10px 20px;border-radius:10px;border:none;font-family:'DM Sans',sans-serif;font-size:.83rem;font-weight:600;cursor:pointer;transition:all .25s;text-decoration:none;letter-spacing:.2px;white-space:nowrap}
        .btn-gold{
            background:linear-gradient(135deg, var(--gold), var(--gold-2));
            color:#0f1035;
            box-shadow:0 4px 18px var(--gold-glow);
        }
        .btn-gold:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(226,185,90,.5)}
        .btn-ghost{background:rgba(255,255,255,.07);color:var(--text-2);border:1px solid var(--rim)}
        .btn-ghost:hover{border-color:var(--rim-2);color:var(--text);background:rgba(255,255,255,.11)}
        .btn-blue{background:rgba(91,143,248,.15);color:var(--blue-2);border:1px solid rgba(91,143,248,.3)}
        .btn-blue:hover{background:rgba(91,143,248,.28);color:#fff}
        .btn-sm{padding:8px 15px;font-size:.77rem}

        .form-footer{padding:18px 30px;display:flex;align-items:center;justify-content:flex-end;gap:10px;border-top:1px solid var(--rim);background:rgba(13,14,45,.45)}

        /* ── Badges ── */
        .badge{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:20px;font-size:.69rem;font-weight:600;letter-spacing:.4px}
        .badge-gold {background:rgba(226,185,90,.14);color:var(--gold);   border:1px solid rgba(226,185,90,.32)}
        .badge-blue {background:rgba(91,143,248,.13);color:var(--blue-2); border:1px solid rgba(91,143,248,.32)}
        .badge-green{background:rgba(94,217,176,.12);color:var(--success);border:1px solid rgba(94,217,176,.32)}
        .badge-red  {background:rgba(244,123,116,.12);color:var(--danger);border:1px solid rgba(244,123,116,.32)}

        /* Map preview */
        .map-preview{border:1px solid rgba(91,143,248,.22);border-radius:12px;margin-top:14px;background:rgba(91,143,248,.06);display:flex;align-items:center;min-height:56px;padding:12px 16px;gap:12px}
        .map-preview i{font-size:1.1rem;color:var(--blue-2)}
        .map-preview span{font-size:.81rem;color:var(--text-2)}

        .social-tag{display:inline-flex;align-items:center;gap:8px;padding:9px 14px;background:rgba(255,255,255,.05);border:1px solid var(--rim);border-radius:10px;font-size:.79rem;color:var(--text-2)}
        .social-tag a{color:var(--blue-2);font-weight:600;text-decoration:none}
        .social-tag a:hover{color:#fff}

        .empty-state{text-align:center;padding:60px 32px}
        .empty-icon{width:70px;height:70px;border-radius:50%;background:rgba(226,185,90,.1);border:1px solid rgba(226,185,90,.25);display:flex;align-items:center;justify-content:center;margin:0 auto 18px;font-size:1.7rem;color:var(--gold)}
        .empty-title{font-family:'Cormorant Garamond',serif;font-size:1.35rem;font-weight:300;margin-bottom:8px}
        .empty-sub{color:var(--muted);font-size:.84rem;max-width:340px;margin:0 auto 24px;line-height:1.65}

        .empty{color:var(--muted);font-style:italic;font-weight:300}
    </style>
</head>
<body>
                        ...
                        <div class="field">
                            ...
                            <select name="emergencyRelation">
                                ...
                                <%
                                    ...
                                    for(String r : relations){ boolean sel = r.equals(er);
...
function uploadAvatar(input){
    ...
    reader.onload=e=>{ document.getElementById('avatarPreview').innerHTML='<img src="'+e.target.result+'" style="width:96px;height:96px;border-radius:50%;object-fit:cover">'; };
