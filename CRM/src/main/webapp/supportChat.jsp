    ...
    <head>
        ...
        <style>
            ...
            *,*::before,*::after{
            ...
            /* ═══ CONVERSATION PANEL ═══ */
            .conv-panel{
                width:var(--conv-width);
                height:100vh;
                background:rgba(11,18,47,0.9);
            ...
            .unread-badge{
                ...
                min-width:16px;
                text-align:center;
                border:2px solid rgba(11,18,47,0.9);
            }
            .conv-info{
                flex:1;
                min-width:0;
            ...
            .scroll-btn{
                ...
                border-radius:50%;
                border:none;
                background:rgba(79,126,248,0.9);
                color:#fff;
                cursor:pointer;
                display:flex;
                align-items:center;
            ...
            .scroll-btn .unread-scroll-badge{
                position:absolute;
                top:-4px;
                right:-4px;
                background:var(--danger);
    ...
    <body>
        ...
        // ── Typing state ──
            ...
            function updateScrollBadge() {
                ...
                if (unreadScrollCount > 0) {
                    badge.textContent = unreadScrollCount;
                    badge.style.display = '';
                } else
            ...
            function setCustomerOnline(online) {
                ...
                if (cdot)
                    cdot.classList.toggle('show', online);
            }
        ...
        // FIX: pollStatus giờ đã có handler trong servlet
            ...
            function buildAvatarHtml(isMine) {
                ...
                if (url)
                    return '<div class="msg-ava ' + cls + '"><img src="' + url + '" alt="avatar"></div>';
        ...
        // customerChat.jsp và supportChat.jsp — thêm CTX check:
            ...
            async function sendMsg() {
                ...
                fetch(CTX + '/supportChat', {method: 'POST', headers: {'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'}, body: params})
                    ...
                    if (d.success && d.id) {
                        ...
                        if (preview) {
                            preview.textContent = 'You: ' + (text || (hadFile ? '📎 Attachment' : ''));
                            preview.classList.remove('unread');
                        }
                        if (timeel) {
                            const now = new Date();
                        ...
                        if (convListEl && convEl)
                            convListEl.prepend(convEl);
                    } else {
                        if (tempEl)
                    ...
                    newMsgs.forEach(m => {
                        if (!renderedIds.has(m.id)) {
                            ...
                            if (m.senderId !== MY_ID) {
                                ...
                                if (preview) {
                                    preview.textContent = msg.length > 42 ? msg.substring(0, 42) + '…' : msg;
                                    preview.classList.remove('unread');
                                }
                                if (timeel) {
                                    const t = m.time || '';
                                    timeel.textContent = t ? t.substring(0, 5) : '';
                                }
                            }
                        }
                    });
            ...
            function pollUpdates() {
                ...
                fetch(CTX + '/supportChat?action=pollUpdates&customerId=' + SEL_CID)
                    ...
                    allMsgs.forEach(m => {
                        ...
                        // Read/Delivered tick
                        if (isMine && !m.recalled) {
                            updateMsgStatus(existingRow, m.read, m.delivered);
                        }
                    });
                }).catch(() => {
                });
            ...
            function pollSidebar() {
                fetch(CTX + '/supportChat?action=pollSidebar')
                    ...
                    list.forEach(item => {
                        ...
                        if (item.customerId === SEL_CID) {
                            const p = document.getElementById('preview-' + item.customerId);
                            if (p)
                        ...
                        if (cdot && item.isOnline !== undefined)
                            cdot.classList.toggle('show', !!item.isOnline);
                    });
                    const convListEl = document.getElementById('convList');
                    if (!convListEl)
                        return;
            ...
            function pinMsg(btn) {
                ...
                if (!row)
                    return;
                const bubble = row.querySelector('.msg-bubble');
                if (bubble.classList.contains('recalled')) {
