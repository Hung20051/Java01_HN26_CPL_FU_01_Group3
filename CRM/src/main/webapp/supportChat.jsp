    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .chat-ava{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:.95rem;font-weight:700;flex-shrink:0;position:relative;}
    ...
    <div class="conv-panel">
        ...
        <div class="conv-list" id="convList">
            ...
            <div class="conv-item <%=active?"active":""%>" onclick="openChat(<%=cid%>)"
                ...
                <div class="c-ava">
                    <div style="width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:.88rem;font-weight:700;color:#fff;" class="ava-<%=colorIdx%8%>"><%=initL%></div>
    ...
    <div class="chat-area">
        ...
        <div class="chat-msgs" id="chatMsgs">
            ...
            <div class="msg-row <%=mine?"mine":"other"%>" data-id="<%=m.getId()%>" data-mine="<%=mine%>"
                ...
                <div class="msg-ava <%=mine?"me":"ava-"+selColor%>"><%=(mine?me.getFullName():selName).substring(0,1).toUpperCase()%></div>
...
function buildMsgRow(m, isMine) {
    ...
    if (m.reactions && Array.isArray(m.reactions)) {
        m.reactions.forEach(rx => {
            if (rx.count > 0) {
                reactionsHtml += '<div class="reaction-chip ' + (rx.mine ? 'mine' : '') + '" ' +
                    'data-emoji="' + rx.emoji + '" ' +
    ...
    .then(newMsgs => {
        ...
        newMsgs.forEach(m => {
            if (!renderedIds.has(m.id)) {
                ...
                if (m.senderId !== MY_ID) { appendMsg(m); appended = true; }
            }
        });
        ...
        list.forEach(item => {
            ...
            if (badge) {
                if (item.unreadCount > 0 && item.customerId !== SEL_CID) {
                    badge.textContent = item.unreadCount; badge.style.display = '';
...
function pinMsg(btn) {
    ...
    fetch(CTX + '/supportChat', {
        method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},
        body: new URLSearchParams({action:'pin', messageId:msgId, customerId:SEL_CID})
    }).catch(() => {});
}
...
function clearPin() {
    ...
    if (oldId) {
        fetch(CTX + '/supportChat', {
            method:'POST',
            headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},
            body: new URLSearchParams({action:'pin', messageId:oldId, customerId:SEL_CID})
        }).catch(() => {});
    }
}
...
function addReaction(btn, emoji) {
    ...
    fetch(CTX + '/supportChat', {
        method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},
        body: new URLSearchParams({action:'react', messageId:msgId, emoji:emoji, customerId:SEL_CID})
    }).catch(() => {});
}
...
function toggleReactionChip(chip, emoji, msgId) {
    ...
    fetch(CTX + '/supportChat', {
        method:'POST',
        headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},
        body: new URLSearchParams({action:'react', messageId:mid, emoji:emoji, customerId:SEL_CID})
    }).catch(() => {});
}
