...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .sb-user:hover{background:rgba(129,140,248,0.18)}
        ...
        .topbar-title{font-size:1rem;font-weight:700;color:var(--text-h)}.topbar-sub{font-size:.75rem;color:var(--text-s);margin-top:1px}
        .content{padding:24px 28px;flex:1}
        @keyframes cardIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:none}}
        /* WELCOME BANNER */
        .welcome{background:linear-gradient(135deg,#4f46e5,#7c3aed);border-radius:18px;padding:26px 28px;color:#fff;display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;animation:cardIn .4s ease;}
        .welcome-title{font-size:1.25rem;font-weight:800}.welcome-sub{font-size:.82rem;opacity:.8;margin-top:4px}
        .welcome-time{font-size:.75rem;opacity:.6;margin-top:8px;display:flex;align-items:center;gap:6px}
        .welcome-btn{background:rgba(255,255,255,0.18);border:1px solid rgba(255,255,255,0.3);color:#fff;padding:9px 18px;border-radius:10px;text-decoration:none;font-size:.8rem;font-weight:700;white-space:nowrap;transition:all .18s;}
        .welcome-btn:hover{background:rgba(255,255,255,0.28);color:#fff}
        /* STAT CARDS */
        .stats{display:grid;grid-template-columns:repeat(5,1fr);gap:14px;margin-bottom:24px;}
        .sc{border-radius:16px;padding:18px 16px;position:relative;overflow:hidden;color:#fff;transition:all .22s;animation:cardIn .45s ease both;cursor:pointer;text-decoration:none;display:block;}
        .sc:hover{transform:translateY(-3px);box-shadow:0 12px 32px rgba(0,0,0,0.18)}
        .sc::after{content:'';position:absolute;width:70px;height:70px;border-radius:50%;background:rgba(255,255,255,0.12);top:-18px;right:-18px;}
        .sc-blue{background:var(--blue);box-shadow:0 4px 20px rgba(37,99,235,0.3)}.sc-info{background:var(--info);box-shadow:0 4px 20px rgba(2,132,199,0.3)}
        .sc-amber{background:var(--amber);box-shadow:0 4px 20px rgba(217,119,6,0.3)}.sc-green{background:var(--green);box-shadow:0 4px 20px rgba(22,163,74,0.3)}
        .sc-red{background:var(--red);box-shadow:0 4px 20px rgba(220,38,38,0.3)}
        .sc:nth-child(1){animation-delay:.04s}.sc:nth-child(2){animation-delay:.08s}.sc:nth-child(3){animation-delay:.12s}.sc:nth-child(4){animation-delay:.16s}.sc:nth-child(5){animation-delay:.20s}
        .sc-icon{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:.88rem;margin-bottom:10px;background:rgba(255,255,255,0.2);position:relative;z-index:1;}
        .sc-val{font-size:1.8rem;font-weight:800;color:#fff;line-height:1;letter-spacing:-1px;position:relative;z-index:1}
        .sc-lbl{color:rgba(255,255,255,0.85);font-size:.72rem;font-weight:600;margin-top:4px;position:relative;z-index:1}
        /* GRID 2 COL */
        .grid2{display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-bottom:24px;}
        .card{background:var(--bg-card);border:1px solid var(--border-light);border-radius:16px;overflow:hidden;animation:cardIn .5s .2s ease both;}
        ...
        .shortcut-btn{display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:12px;border:1px solid var(--border-light);text-decoration:none;color:var(--text-b);transition:all .18s;font-size:.82rem;font-weight:600;}
        .shortcut-btn:hover{background:var(--primary-light);border-color:rgba(99,102,241,0.3);color:var(--primary-2);}
        .shortcut-btn i{width:32px;height:32px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:.82rem;flex-shrink:0;}
        .shortcut-btn.blue i{background:#dbeafe;color:var(--blue)}.shortcut-btn.green i{background:#d1fae5;color:var(--green)}
        .shortcut-btn.purple i{background:#ede9fe;color:var(--purple)}.shortcut-btn.teal i{background:#ccfbf1;color:var(--teal)}
        .shortcut-btn.amber i{background:#fef3c7;color:var(--amber)}.shortcut-btn.red i{background:#fee2e2;color:var(--red)}
        /* INFO ITEMS */
        .info-item{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--border-light2);}
        .info-item:last-child{border-bottom:none}
        .info-label{font-size:.78rem;color:var(--text-s)}
        .info-val{font-size:.82rem;font-weight:600;color:var(--text-h)}
    </style>
</head>
<body>
<!-- SIDEBAR -->
