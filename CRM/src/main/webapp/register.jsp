...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        body{font-family:'Sora',sans-serif;background:var(--navy);color:var(--text);min-height:100vh;display:flex;flex-direction:column}
        ::-webkit-scrollbar{width:4px}
        ::-webkit-scrollbar-track{background:transparent}
        ::-webkit-scrollbar-thumb{background:rgba(79,70,229,0.3);border-radius:4px}

        /* ── NAVBAR ── */
        .navbar{display:flex;justify-content:space-between;align-items:center;padding:20px 64px;border-bottom:1px solid var(--border);background:rgba(11,20,55,0.8);backdrop-filter:blur(16px);position:sticky;top:0;z-index:10}
        .navbar-brand{display:flex;align-items:center;gap:10px;font-size:1.1rem;font-weight:700;color:white;text-decoration:none}
        .brand-icon{width:36px;height:36px;background:linear-gradient(135deg,var(--accent),var(--accent-2));border-radius:10px;display:flex;align-items:center;justify-content:center;color:white;font-size:.85rem;box-shadow:0 4px 12px var(--accent-glow)}
        .navbar-links{display:flex;align-items:center;gap:20px}
        .nav-link{font-size:.82rem;font-weight:500;color:var(--muted);text-decoration:none;transition:color .25s}
        .nav-link:hover{color:white}
        .btn-nav{display:inline-flex;align-items:center;gap:7px;padding:8px 18px;background:rgba(79,126,248,.15);border:1px solid rgba(79,126,248,.3);color:var(--accent-2);text-decoration:none;font-size:.82rem;font-weight:600;border-radius:100px;transition:all .25s}
        .btn-nav:hover{background:rgba(79,126,248,.25);color:white}

        /* ── PAGE WRAP ── */
        .page-wrap{flex:1;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;padding:48px 20px}
        .orb{position:absolute;border-radius:50%;filter:blur(100px);pointer-events:none}
        .orb-1{width:550px;height:550px;background:radial-gradient(circle,rgba(79,126,248,.18) 0%,transparent 70%);top:-120px;right:5%;animation:orbFloat 10s ease-in-out infinite}
        .orb-2{width:380px;height:380px;background:radial-gradient(circle,rgba(52,211,153,.1) 0%,transparent 70%);bottom:-60px;left:5%;animation:orbFloat 14s ease-in-out infinite reverse}
        .orb-3{width:300px;height:300px;background:radial-gradient(circle,rgba(167,139,250,.1) 0%,transparent 70%);top:50%;left:30%;animation:orbFloat 18s ease-in-out infinite}
        @keyframes orbFloat{0%,100%{transform:translate(0,0)}50%{transform:translate(20px,-25px)}}

        /* ── WRAPPER ── */
        .register-wrapper{display:flex;align-items:center;gap:56px;width:100%;max-width:960px;position:relative;z-index:1}

        /* ── INFO PANEL ── */
        .info-panel{flex:1;animation:fadeUp .7s .2s cubic-bezier(.4,0,.2,1) both}
        @keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}
        .info-eyebrow{display:inline-flex;align-items:center;gap:8px;font-size:.68rem;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--green);margin-bottom:16px}
        .info-eyebrow::before{content:'';display:inline-block;width:18px;height:2px;background:var(--green);border-radius:2px}
        .info-title{font-size:clamp(1.6rem,2.5vw,2.4rem);font-weight:800;line-height:1.15;letter-spacing:-.8px;color:white;margin-bottom:14px}
        .info-title .highlight{background:linear-gradient(135deg,#6ef0ff 0%,var(--green) 60%,#a78bfa 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
        .info-sub{font-size:.88rem;font-weight:300;color:var(--muted);line-height:1.8;margin-bottom:32px}
        .step-list{display:flex;flex-direction:column;gap:14px}
        .step-item{display:flex;align-items:flex-start;gap:14px;padding:12px 16px;background:rgba(255,255,255,.03);border:1px solid var(--border);border-radius:14px;transition:all .25s}
        .step-item:hover{background:rgba(52,211,153,.06);border-color:rgba(52,211,153,.2)}
        .step-num{width:28px;height:28px;flex-shrink:0;border-radius:8px;background:rgba(52,211,153,.12);color:var(--green);display:flex;align-items:center;justify-content:center;font-size:.75rem;font-weight:700}
        .step-text{font-size:.83rem;color:var(--text-2);line-height:1.5}
        .step-text strong{color:white;display:block;margin-bottom:2px;font-weight:600}

        /* ── REGISTER CARD ── */
        .register-card{flex-shrink:0;width:440px;background:rgba(17,26,66,.75);border:1px solid var(--border);border-radius:28px;padding:40px 38px;backdrop-filter:blur(24px);box-shadow:0 0 0 1px rgba(52,211,153,.06),0 32px 80px rgba(0,0,0,.45),inset 0 1px 0 rgba(255,255,255,.07);animation:cardIn .6s cubic-bezier(.4,0,.2,1) both}
        @keyframes cardIn{from{opacity:0;transform:translateY(28px) scale(.97)}to{opacity:1;transform:translateY(0) scale(1)}}
        .card-badge{display:inline-flex;align-items:center;gap:6px;background:rgba(52,211,153,.1);border:1px solid rgba(52,211,153,.25);color:var(--green);font-size:.68rem;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;padding:5px 12px;border-radius:100px;margin-bottom:18px}
        .card-title{font-size:1.65rem;font-weight:800;color:white;letter-spacing:-.5px;margin-bottom:4px;line-height:1.15}
        .card-sub{font-size:.82rem;color:var(--muted);font-weight:300;margin-bottom:24px}

        .alert{display:flex;align-items:center;gap:9px;padding:11px 14px;border-radius:10px;font-size:.82rem;font-weight:500;margin-bottom:16px}
        .alert-error{background:rgba(248,113,113,.12);border:1px solid rgba(248,113,113,.25);color:var(--danger)}

        .form-row{display:grid;grid-template-columns:1fr 1fr;gap:14px}
        .form-group{margin-bottom:15px}
        .form-label{display:block;font-size:.72rem;font-weight:600;color:var(--text-2);margin-bottom:7px;letter-spacing:.5px;text-transform:uppercase}
        .required{color:var(--danger);margin-left:2px}
        .input-wrap{position:relative}
        .input-icon{position:absolute;left:13px;top:50%;transform:translateY(-50%);color:var(--muted);font-size:.8rem;pointer-events:none;transition:color .25s}
        .form-input{width:100%;padding:11px 13px 11px 38px;background:rgba(255,255,255,.05);border:1px solid var(--border);border-radius:11px;color:white;font-family:'Sora',sans-serif;font-size:.85rem;outline:none;transition:all .25s}
        .form-input::placeholder{color:var(--muted)}
        .form-input:focus{border-color:var(--border-focus);background:rgba(79,126,248,.07);box-shadow:0 0 0 3px rgba(79,126,248,.12)}
        .input-wrap:focus-within .input-icon{color:var(--accent-2)}
        .toggle-pass{position:absolute;right:13px;top:50%;transform:translateY(-50%);color:var(--muted);cursor:pointer;font-size:.8rem;transition:color .25s}
        .toggle-pass:hover{color:var(--accent-2)}

        .checkbox-group{margin-top:4px;margin-bottom:6px}
        .checkbox-label{display:flex;align-items:flex-start;gap:10px;font-size:.78rem;color:var(--muted);cursor:pointer;line-height:1.5}
        .checkbox-label input[type=checkbox]{width:15px;height:15px;flex-shrink:0;margin-top:2px;accent-color:var(--green)}
        .terms-link{color:var(--green);text-decoration:none;font-weight:600;cursor:pointer;background:none;border:none;font-family:inherit;font-size:inherit;padding:0}
        .terms-link:hover{color:white;text-decoration:underline}

        .btn-primary{width:100%;padding:13px;background:linear-gradient(135deg,#22c97a,var(--green));border:none;border-radius:12px;color:white;font-family:'Sora',sans-serif;font-size:.88rem;font-weight:700;cursor:pointer;transition:all .3s cubic-bezier(.4,0,.2,1);box-shadow:0 6px 24px var(--green-glow);display:flex;align-items:center;justify-content:center;gap:8px;margin-top:18px}
        .btn-primary:hover{transform:translateY(-2px);box-shadow:0 14px 36px rgba(52,211,153,.4)}
        .btn-primary:active{transform:translateY(0)}

        .login-cta{text-align:center;margin-top:18px;font-size:.8rem;color:var(--muted)}
        .login-cta a{color:var(--accent-2);text-decoration:none;font-weight:600;transition:color .2s}
        ...
        ════════════════════════════════════════ */
        .t-overlay{display:none;position:fixed;inset:0;background:rgba(15,23,42,0.55);z-index:1000;align-items:center;justify-content:center;padding:16px;backdrop-filter:blur(6px)}
        ...
        .t-head-left{display:flex;align-items:center;gap:12px}
        .t-icon{
            width:40px;height:40px;border-radius:12px;
            background:#d1fae5;border:1px solid #a7f3d0;
            display:flex;align-items:center;justify-content:center;
            color:var(--m-green);font-size:1rem;flex-shrink:0;
        }
        .t-title{font-size:1.02rem;font-weight:800;color:var(--m-text);line-height:1.2}
        .t-version{font-size:.68rem;color:var(--m-text-4);margin-top:2px}
        .t-close{
            background:#f1f5f9;border:1px solid var(--m-border);
            border-radius:9px;width:32px;height:32px;
            cursor:pointer;display:flex;align-items:center;justify-content:center;
            color:var(--m-text-3);font-size:.8rem;transition:all .2s;flex-shrink:0;
        }
        .t-close:hover{background:#fee2e2;border-color:#fca5a5;color:var(--m-red)}
        ...
        .t-tabs::-webkit-scrollbar{height:0}
        .t-tab{
            font-size:.72rem;font-weight:600;color:var(--m-text-3);
            background:none;border:none;cursor:pointer;
            padding:10px 12px;border-bottom:2px solid transparent;
            white-space:nowrap;transition:all .15s;font-family:'Sora',sans-serif;
        }
        .t-tab:hover{color:var(--m-text-2)}
        .t-tab.on{color:var(--m-primary);border-bottom-color:var(--m-primary);font-weight:700}

        /* Body */
        .t-body{flex:1;overflow-y:auto;padding:22px 24px;min-height:0;background:var(--m-bg)}
        .t-body::-webkit-scrollbar{width:4px}
        .t-body::-webkit-scrollbar-track{background:transparent}
        .t-body::-webkit-scrollbar-thumb{background:rgba(79,70,229,0.2);border-radius:4px}

        .t-sec{display:none}
        .t-sec.on{display:block}

        .t-sec-title{
            font-size:.9rem;font-weight:700;color:var(--m-text);
            margin-bottom:14px;display:flex;align-items:center;gap:10px;
        }
        .t-sec-num{
            width:24px;height:24px;border-radius:7px;
            background:#ede9fe;color:var(--m-primary);
            font-size:.7rem;font-weight:700;
            display:flex;align-items:center;justify-content:center;flex-shrink:0;
        }

        .t-p{font-size:.82rem;color:var(--m-text-2);line-height:1.8;margin-bottom:12px}
        ...
        .t-clause-h{
            font-size:.76rem;font-weight:700;color:var(--m-text);
            margin-bottom:6px;display:flex;align-items:center;gap:7px;
        }
        .t-clause-h::before{content:'';width:3px;height:12px;background:var(--m-primary);border-radius:2px;display:inline-block;flex-shrink:0}
        .t-clause-t{font-size:.78rem;color:var(--m-text-2);line-height:1.75}

        /* Tags */
        .t-tag{display:inline-block;font-size:.65rem;font-weight:700;padding:2px 8px;border-radius:100px;margin-right:4px}
        .t-tag-blue  {background:#dbeafe;color:var(--m-blue);border:1px solid #bfdbfe}
        .t-tag-amber {background:#fef3c7;color:var(--m-amber);border:1px solid #fde68a}
        .t-tag-red   {background:#fee2e2;color:var(--m-red);border:1px solid #fca5a5}
        .t-tag-green {background:#d1fae5;color:var(--m-green);border:1px solid #a7f3d0}
        ...
        .t-notice p{font-size:.78rem;color:#92400e;margin:0;line-height:1.6}
        ...
        .t-contact p{font-size:.76rem;color:var(--m-text-3);margin:0;line-height:1.75}
        .t-contact a{color:var(--m-primary);text-decoration:none;font-weight:600}
        .t-contact a:hover{color:var(--m-primary-2);text-decoration:underline}
        ...
        .t-foot{
            ...
            flex-shrink:0;background:var(--m-bg-2);
        }
        .t-progress{font-size:.72rem;color:var(--m-text-3);display:flex;align-items:center;gap:8px}
        .t-dots{display:flex;gap:4px}
        .t-dot{width:6px;height:6px;border-radius:50%;background:var(--m-border);transition:all .2s;cursor:pointer}
        .t-dot.on{background:var(--m-primary);width:18px;border-radius:3px}

        .t-foot-btns{display:flex;gap:8px}
        .t-btn-prev,.t-btn-next{
            padding:7px 14px;border-radius:9px;
            font-family:'Sora',sans-serif;font-size:.75rem;font-weight:600;
            cursor:pointer;transition:all .2s;
            border:1px solid var(--m-border);
            background:#fff;color:var(--m-text-2);
        }
        .t-btn-prev:hover,.t-btn-next:hover{background:var(--m-bg-3);color:var(--m-text);border-color:#cbd5e1}
        .t-btn-accept{
            padding:7px 20px;border-radius:9px;
            font-family:'Sora',sans-serif;font-size:.75rem;font-weight:700;
            cursor:pointer;transition:all .2s;border:none;
            background:var(--m-primary);color:#fff;display:none;
        }
        .t-btn-accept:hover{background:var(--m-primary-2);transform:translateY(-1px);box-shadow:0 4px 12px rgba(79,70,229,0.3)}
        .t-btn-accepted{
            padding:7px 20px;border-radius:9px;
            font-family:'Sora',sans-serif;font-size:.75rem;font-weight:700;
            border:1px solid #a7f3d0;background:#d1fae5;color:var(--m-green);display:none;
        }
    </style>
</head>
<body>
...
<script>
    ...
    function navSec(dir) {
        ...
        if (next >= 0 && next < TOTAL) showSec(next);
    }
    ...
    document.addEventListener('keydown', e => {
        if (!document.getElementById('termsOverlay').classList.contains('open')) return;
        if (e.key === 'Escape')     closeTerms();
        if (e.key === 'ArrowRight') navSec(1);
        if (e.key === 'ArrowLeft')  navSec(-1);
    });
</script>
</body>
</html>
