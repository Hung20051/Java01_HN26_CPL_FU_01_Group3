...
<head>
    ...
    <title>Danh sách thiết bị - CRM System</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'Segoe UI', sans-serif; background: #f0f2f5; display: flex; min-height: 100vh; }
        .sidebar { width: 200px; min-height: 100vh; background: #1a1a2e; display: flex; flex-direction: column; position: fixed; }
        .sidebar-brand { padding: 20px 16px; color: white; font-size: 1rem; font-weight: 700; border-bottom: 1px solid rgba(255,255,255,0.1); }
        .sidebar-brand i { color: #4ade80; margin-right: 8px; }
        .sidebar-nav { flex: 1; padding: 12px 0; }
        .nav-item { display: flex; align-items: center; gap: 10px; padding: 11px 20px; color: rgba(255,255,255,0.7);
            text-decoration: none; font-size: 0.875rem; transition: all 0.2s; border-left: 3px solid transparent; }
        .nav-item:hover, .nav-item.active { color: white; background: rgba(255,255,255,0.08); border-left-color: #4ade80; }
        ...
        .user-badge { background: white; border: 1px solid #e2e8f0; border-radius: 20px;
        ...
        .search-box { flex: 1; min-width: 200px; padding: 8px 14px; border: 1px solid #e2e8f0;
            border-radius: 8px; font-size: 0.875rem; outline: none; }
        .search-box:focus { border-color: #6366f1; }
        .select-box { padding: 8px 12px; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 0.875rem; outline: none; background: white; }
        .btn { padding: 8px 16px; border-radius: 8px; font-size: 0.875rem; font-weight: 500;
            border: none; cursor: pointer; display: flex; align-items: center; gap: 6px; text-decoration: none; }
        ...
        th { padding: 12px 14px; text-align: left; color: #64748b; font-weight: 600;
            font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.4px; border-bottom: 1px solid #e2e8f0; }
        td { padding: 12px 14px; border-bottom: 1px solid #f1f5f9; color: #374151; }
        tr:last-child td { border-bottom: none; }
        tr:hover td { background: #fafbff; }

        .expand-btn { background: none; border: none; cursor: pointer; color: #6366f1; font-size: 1rem; padding: 4px; }
        .action-btns { display: flex; gap: 6px; }
        .btn-edit   { background: #f59e0b; color: white; padding: 5px 10px; font-size: 0.78rem; border-radius: 6px; border: none; cursor: pointer; }
        .btn-delete { background: #ef4444; color: white; padding: 5px 10px; font-size: 0.78rem; border-radius: 6px; border: none; cursor: pointer; }
        .btn-add-unit { background: #0891b2; color: white; padding: 5px 10px; font-size: 0.78rem; border-radius: 6px; border: none; cursor: pointer; }

        /* Expand row for serial numbers */
        .expand-row { display: none; }
        .expand-row.show { display: table-row; }
        .expand-content { background: #f8fafc; padding: 12px 20px; }
        .serial-list { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px; }
        .serial-badge { padding: 4px 10px; border-radius: 6px; font-size: 0.78rem; font-weight: 500; }
        .s-available { background: #d1fae5; color: #065f46; }
        .s-inuse     { background: #dbeafe; color: #1e40af; }
        .s-faulty    { background: #fef3c7; color: #92400e; }
        .s-retired   { background: #f3f4f6; color: #6b7280; }

        .pagination { display: flex; justify-content: center; align-items: center; gap: 6px; padding: 16px; }
        .page-btn { padding: 6px 12px; border: 1px solid #e2e8f0; border-radius: 6px; background: white;
            font-size: 0.82rem; cursor: pointer; text-decoration: none; color: #374151; }
        .page-btn.active { background: #6366f1; color: white; border-color: #6366f1; }
        .page-btn.disabled { opacity: 0.4; pointer-events: none; }
        .paging-info { color: #94a3b8; font-size: 0.82rem; }

        .modal-overlay { display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; }
        .modal-overlay.show { display: flex; }
        .modal { background: white; border-radius: 12px; padding: 28px; width: 460px; max-width: 95vw; box-shadow: 0 20px 60px rgba(0,0,0,0.2); }
        .modal h3 { font-size: 1.1rem; font-weight: 700; margin-bottom: 20px; color: #1e293b; text-align: center; }
        .form-group { margin-bottom: 14px; }
        .form-group label { display: block; font-size: 0.82rem; font-weight: 600; color: #374151; margin-bottom: 5px; }
        .form-group input, .form-group select { width: 100%; padding: 9px 12px; border: 1px solid #d1d5db; border-radius: 8px; font-size: 0.875rem; outline: none; }
        .form-group input:focus, .form-group select:focus { border-color: #6366f1; }
        .modal-btns { display: flex; gap: 10px; margin-top: 20px; }
        .modal-btns button { flex: 1; padding: 10px; border-radius: 8px; border: none; cursor: pointer; font-size: 0.9rem; font-weight: 600; }
        .btn-save { background: #10b981; color: white; }
        .btn-cancel { background: #94a3b8; color: white; }
        .alert { padding: 10px 14px; border-radius: 8px; margin-bottom: 16px; font-size: 0.875rem; display: flex; align-items: center; gap: 8px; }
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<%= ctx %>/numberEquipment"  class="nav-item active"><i class="fas fa-desktop"></i> Danh sách thiết bị</a>
...
<main class="main">
    ...
    <div class="toolbar">
        <input class="search-box" type="text" name="keyword" placeholder="Nhập từ khoá tìm kiếm..." value="<%= keyword != null ? keyword : "" %>">
    ...
    <div class="card">
        <table>
            ...
            <tr><td colspan="7" style="text-align:center;color:#94a3b8;padding:30px">Không có thiết bị nào</td></tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    <span style="font-size:0.75rem;color:#94a3b8"> sẵn/tổng</span>
                </td>
                <td>
            ...
            <!-- Expand Row for Serial Numbers - loaded via AJAX or pre-loaded -->
                ...
                <td colspan="7">
                    <div class="expand-content">
                        ...
                        <div class="serial-list">
                            <%
                                // Show units if this is the detail view
        ...
        <div class="pagination">
            <span class="paging-info">Trang <%= currentPage %> / <%= totalPages %> (Tổng <%= total %> models)</span>
            &nbsp;
...
<div class="modal">
    ...
    <form method="post" action="<%= ctx %>/numberEquipment">
        ...
        <div class="form-group">
            <label>Model Name * (Tối thiểu 3 ký tự)</label>
        ...
        <div class="form-group">
            <label>Serial Number * (đầu tiên)</label>
            <input type="text" name="serialNumber" required placeholder="VD: DAI-VRV4-003">
        </div>
...
<div class="modal">
    ...
    <form method="post" action="<%= ctx %>/numberEquipment">
        ...
        <div class="form-group">
            ...
            <input type="text" name="serialNumber" required placeholder="VD: DAI-VRV4-004">
        </div>
        <div class="modal-btns">
            <button type="submit" class="btn-save"><i class="fas fa-plus"></i> Nhập kho</button>
            <button type="button" class="btn-cancel" onclick="closeModal('addUnitModal')">✕ Hủy</button>
        </div>
    </form>
</div>
</div>
...
<div class="modal" style="text-align:center">
    ...
    <h3>Xác nhận xóa</h3>
    <p style="color:#64748b;margin:10px 0 20px">Bạn có chắc muốn xóa thiết bị:<br><strong id="deleteEqName"></strong>?</p>
    <form method="post" action="<%= ctx %>/numberEquipment">
        <input type="hidden" name="action" value="delete">
        <input type="hidden" name="id" id="deleteId">
        <div class="modal-btns">
            <button type="submit" style="background:#ef4444;color:white;flex:1;padding:10px;border-radius:8px;border:none;cursor:pointer;font-weight:600">✓ Xác nhận</button>
            <button type="button" onclick="closeModal('deleteModal')" style="background:#94a3b8;color:white;flex:1;padding:10px;border-radius:8px;border:none;cursor:pointer;font-weight:600">✕ Hủy</button>
        </div>
    </form>
</div>
</div>
