...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .breadcrumb-cur{color:var(--text-m);font-weight:600;font-family:'Courier New',monospace}
        .btn-back{display:inline-flex;align-items:center;gap:7px;padding:8px 16px;background:#fff;color:var(--text-m);border:1.5px solid var(--border-light);text-decoration:none;font-size:.81rem;font-weight:600;border-radius:9px;transition:all .2s;}
        ...
        /* ── ALERTS ── */
        .alert-box{display:flex;align-items:flex-start;gap:12px;padding:12px 18px;border-radius:12px;margin-bottom:18px;font-size:.82rem;animation:fadeSlideIn .4s ease both;}
        ...
        /* ── LAYOUT ── */
        .grid-inv{display:grid;grid-template-columns:3fr 2fr;gap:18px;align-items:start;}
        ...
        .inv-header{
            ...
            animation:cardIn .5s ease both;position:relative;overflow:hidden;
        }
        .inv-header::after{content:'';position:absolute;width:120px;height:120px;border-radius:50%;background:rgba(255,255,255,0.1);top:-30px;right:-30px;}
        .inv-header::before{content:'';position:absolute;width:70px;height:70px;border-radius:50%;background:rgba(255,255,255,0.07);bottom:-15px;right:50px;}
        .inv-title{font-size:1.35rem;font-weight:800;color:#fff;letter-spacing:-.5px;margin-bottom:4px;display:flex;align-items:center;gap:10px;position:relative;z-index:1;}
        .inv-title i{color:rgba(255,255,255,0.8)}
        .inv-code-hd{font-family:'Courier New',monospace;font-size:.94rem;color:rgba(255,255,255,0.9);font-weight:700;letter-spacing:.5px;position:relative;z-index:1;}
        .inv-date{color:rgba(255,255,255,0.65);font-size:.77rem;margin-top:5px;position:relative;z-index:1;}
        .inv-company{text-align:right;font-size:.79rem;color:rgba(255,255,255,0.8);line-height:1.8;position:relative;z-index:1;}
        .inv-company strong{color:#fff}

        /* ── CARDS ── */
        .card{background:var(--bg-card);border:1px solid var(--border-light);border-radius:16px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,0.05);margin-bottom:16px;animation:cardIn .5s ease both;}
        .card:nth-child(1){animation-delay:.1s}
        .card:nth-child(2){animation-delay:.15s}
        .card-hd{display:flex;align-items:center;gap:10px;padding:13px 18px;border-bottom:1px solid var(--border-light2);background:#fafbff;}
        .card-hd-icon{width:30px;height:30px;border-radius:8px;background:var(--primary-light);color:var(--primary-2);display:flex;align-items:center;justify-content:center;font-size:.78rem;flex-shrink:0;}
        ...
        /* Item type badge */
        .item-type{display:inline-flex;align-items:center;padding:3px 9px;border-radius:20px;font-size:.67rem;font-weight:700;}
        .it-part   {background:#e0f2fe;color:var(--info);border:1px solid #bae6fd}
        .it-service{background:#d1fae5;color:var(--green);border:1px solid #a7f3d0}
        .it-equip  {background:#fef3c7;color:#92400e;border:1px solid #fde68a}
        .it-other  {background:var(--primary-light);color:var(--purple);border:1px solid rgba(124,58,237,0.2)}

        /* Summary rows */
        .sum-wrap{padding:14px 18px;border-top:1px solid var(--border-light2);background:#fafbff}
        .sum-inner{max-width:280px;margin-left:auto}
        .sum-row{display:flex;justify-content:space-between;padding:7px 0;font-size:.83rem;border-bottom:1px solid var(--border-light2);color:var(--text-b);}
        .sum-row .lbl{color:var(--text-s)}
        .sum-row:last-child{border:none;border-top:1px solid var(--border-light);font-size:1.02rem;font-weight:800;color:var(--primary-2);padding-top:11px;margin-top:3px;}
        .sum-row:last-child .lbl{color:var(--text-h)}
        ...
        .info-item .lbl{font-size:.67rem;color:var(--text-s);font-weight:700;text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px;}
        .info-item .val{font-size:.83rem;color:var(--text-b);font-weight:500}
        .info-item .val a{color:var(--primary-2);text-decoration:none;font-weight:700;font-family:'Courier New',monospace}
        .info-item .val a:hover{color:var(--primary);text-decoration:underline}

        /* Amount display */
        .amount-box{background:#fafbff;border:1px solid var(--border-light);border-radius:12px;padding:16px;margin-bottom:14px;}
        .amount-lbl{font-size:.67rem;color:var(--text-s);font-weight:700;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px;}
        .amount-val{font-size:1.75rem;font-weight:800;line-height:1;letter-spacing:-1px;}
        .amount-val.unpaid{color:var(--red)}
        .amount-val.paid  {color:var(--green)}
        .amount-sub{font-size:.72rem;color:var(--text-s);margin-top:6px;display:flex;align-items:center;gap:5px}

        /* ── PAYMENT SECTION ── */
        .pay-section{margin-top:14px}
        .pay-title{font-size:.67rem;font-weight:700;color:var(--text-s);text-transform:uppercase;letter-spacing:.8px;margin-bottom:10px;display:flex;align-items:center;gap:6px}
        .pay-btns{display:flex;flex-direction:column;gap:9px}
        .btn-pay{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;padding:12px;color:#fff;border:none;border-radius:11px;font-size:.875rem;font-weight:700;cursor:pointer;transition:all .25s;font-family:'Sora',sans-serif;}
        .btn-pay-cash {background:var(--green);box-shadow:0 3px 12px rgba(22,163,74,0.25)}
        .btn-pay-vnpay{background:linear-gradient(135deg,#f43f5e,#be123c);box-shadow:0 3px 12px rgba(244,63,94,0.25)}
        .btn-pay-cash:hover {background:#15803d;transform:translateY(-2px);box-shadow:0 6px 18px rgba(22,163,74,0.38)}
        .btn-pay-vnpay:hover{transform:translateY(-2px);box-shadow:0 6px 18px rgba(244,63,94,0.38)}

        /* ── MODAL ── */
        .modal-overlay{display:none;position:fixed;inset:0;background:rgba(243,244,249,0.7);backdrop-filter:blur(6px);z-index:999;align-items:center;justify-content:center;}
        .modal-overlay.show{display:flex}
        .modal{background:var(--bg-card);border:1px solid var(--border-light);border-radius:18px;width:440px;max-width:95vw;overflow:hidden;box-shadow:0 16px 50px rgba(79,70,229,0.15);animation:modalIn .25s ease;}
        @keyframes modalIn{from{opacity:0;transform:scale(0.95) translateY(10px)}to{opacity:1;transform:scale(1) translateY(0)}}
        .modal-header{padding:16px 22px;border-bottom:1px solid var(--border-light);display:flex;align-items:center;justify-content:space-between;background:#fafbff;}
        .modal-title{font-size:.93rem;font-weight:700;color:var(--text-h);display:flex;align-items:center;gap:9px}
        .modal-title i{color:var(--green)}
        .modal-close{width:30px;height:30px;border-radius:8px;background:#fff;border:1.5px solid var(--border-light);cursor:pointer;display:flex;align-items:center;justify-content:center;color:var(--text-m);font-size:.9rem;transition:all .2s;}
        .modal-close:hover{background:#fee2e2;color:var(--red);border-color:#fca5a5}
        .modal-body{padding:22px}
        .modal-footer{padding:14px 22px;border-top:1px solid var(--border-light);display:flex;gap:9px;background:#fafbff}

        /* Cash modal */
        .cash-amount-box{background:#d1fae5;border:1px solid #a7f3d0;border-radius:12px;padding:18px;text-align:center;margin-bottom:18px;}
        .cash-amount-lbl{font-size:.71rem;color:#065f46;font-weight:600;margin-bottom:5px}
        .cash-amount-val{font-size:1.85rem;font-weight:800;color:var(--green);letter-spacing:-1px}
        .cash-amount-code{font-family:'Courier New',monospace;font-size:.77rem;color:var(--text-m);margin-top:5px}

        /* Cash steps */
        .cash-steps{list-style:none;margin-bottom:18px}
        .cash-steps li{display:flex;gap:11px;padding:9px 0;border-bottom:1px solid var(--border-light2);font-size:.81rem;color:var(--text-b);}
        .cash-steps li:last-child{border:none}
        .step-num{width:22px;height:22px;border-radius:50%;background:var(--primary);color:#fff;font-size:.67rem;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;}

        .btn-confirm-cash{flex:1;padding:11px;background:var(--green);color:#fff;border:none;border-radius:10px;font-size:.875rem;font-weight:700;cursor:pointer;transition:all .2s;font-family:'Sora',sans-serif;}
        .btn-confirm-cash:hover{background:#15803d;transform:translateY(-1px);box-shadow:0 4px 14px rgba(22,163,74,0.3)}
        .btn-modal-cancel{padding:11px 16px;background:#fff;color:var(--text-m);border:1.5px solid var(--border-light);border-radius:10px;font-size:.875rem;font-weight:600;cursor:pointer;transition:all .2s;font-family:'Sora',sans-serif;}
        .btn-modal-cancel:hover{background:#f3f4f6;border-color:#d1d5db;color:var(--text-b)}
    </style>
</head>
<body>
