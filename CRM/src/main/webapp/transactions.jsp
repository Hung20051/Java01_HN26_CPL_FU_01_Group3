...
<head>
    ...
    <title>Lịch sử giao dịch - CRM System</title>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* SIDEBAR */
        .sidebar { width: 200px; min-height: 100vh; background: #1a1a2e; display: flex; flex-direction: column; position: fixed; top:0; left:0; }
        ...
        .page-title i { color: #6366f1; font-size: 1.3rem; }
        ...
        /* TABS */
        .tabs { display: flex; gap: 4px; background: white; border-radius: 10px; padding: 5px;
            border: 1px solid #e2e8f0; margin-bottom: 20px; width: fit-content; }
        .tab { padding: 8px 18px; border-radius: 7px; font-size: 0.85rem; font-weight: 500;
            cursor: pointer; text-decoration: none; color: #64748b; display: flex; align-items: center; gap: 7px;
            transition: all 0.15s; white-space: nowrap; }
        .tab:hover { background: #f1f5f9; color: #374151; }
        .tab.active { background: #6366f1; color: white; }
        .tab .cnt { background: rgba(255,255,255,0.3); padding: 1px 7px; border-radius: 10px; font-size: 0.75rem; }
        .tab:not(.active) .cnt { background: #f1f5f9; color: #64748b; }
        ...
        .filter-bar input, .filter-bar select {
            ...
            font-size: 0.85rem; outline: none; background: white; color: #374151;
        }
        ...
        th { padding: 11px 14px; text-align: left; color: #64748b; font-weight: 600;
            font-size: 0.76rem; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 1px solid #e2e8f0; }
        ...
        .badge-part      { background: #ede9fe; color: #6d28d9; }
        .badge-equipment { background: #fce7f3; color: #9d174d; }
        .badge-sale   { background: #dbeafe; color: #1e40af; }
        .badge-repair-action { background: #fef3c7; color: #92400e; }
        .badge-import-action { background: #d1fae5; color: #065f46; }

        .order-code { font-family: monospace; font-size: 0.8rem; color: #6366f1; font-weight: 600; }
        .customer-name { font-weight: 500; color: #1e293b; }
        .note-text { color: #94a3b8; font-size: 0.8rem; max-width: 180px; overflow: hidden;
            text-overflow: ellipsis; white-space: nowrap; }
        .time-text { color: #94a3b8; font-size: 0.8rem; white-space: nowrap; }
        ...
        .page-btn { padding: 5px 11px; border: 1px solid #e2e8f0; border-radius: 6px; background: white;
...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<%= ctx %>/transactions"     class="nav-item active"><i class="fas fa-history"></i> Lịch sử giao dịch</a>
...
<main class="main">
    ...
    <div class="tabs">
        <a href="<%= ctx %>/transactions?type=ALL" class="tab <%= "ALL".equals(activeType) ? "active" : "" %>">
            <i class="fas fa-list"></i> Tất cả <span class="cnt"><%= cntAll %></span>
        </a>
        <a href="<%= ctx %>/transactions?type=PURCHASE" class="tab <%= "PURCHASE".equals(activeType) ? "active" : "" %>">
            <i class="fas fa-shopping-cart"></i> Mua hàng <span class="cnt"><%= cntPurchase %></span>
        </a>
        <a href="<%= ctx %>/transactions?type=REPAIR" class="tab <%= "REPAIR".equals(activeType) ? "active" : "" %>">
            <i class="fas fa-tools"></i> Sửa chữa <span class="cnt"><%= cntRepair %></span>
        </a>
        <a href="<%= ctx %>/transactions?type=IMPORT" class="tab <%= "IMPORT".equals(activeType) ? "active" : "" %>">
            <i class="fas fa-box-open"></i> Nhập kho <span class="cnt"><%= cntImport %></span>
        </a>
    </div>
    ...
    <form method="get" action="<%= ctx %>/transactions">
        ...
        <div class="filter-bar">
            <input type="text" name="keyword" placeholder="Tìm kiếm tên linh kiện, thiết bị, khách hàng..."
                   value="<%= keyword %>">
            <select name="itemType">
                <option value="">-- Tất cả loại --</option>
                <option value="PART"      <%= "PART".equals(itemType)      ? "selected" : "" %>>Linh kiện (Part)</option>
                <option value="EQUIPMENT" <%= "EQUIPMENT".equals(itemType) ? "selected" : "" %>>Thiết bị (Equipment)</option>
            </select>
            <input type="date" name="fromDate" value="<%= fromDate %>" title="Từ ngày">
    ...
    <div class="card">
        ...
        <table>
            <thead>
                <tr>
                    ...
                    %>
                    <% if (isPurchase || "ALL".equals(activeType)) { %><th>Khách hàng</th><th>Mã đơn</th><% } %>
                    <% if (isRepair || "ALL".equals(activeType)) { %><th>Đơn sửa #</th><% } %>
                    <th>Thực hiện bởi</th>
                    <th>Ghi chú</th>
                    <th>Thời gian</th>
                </tr>
            ...
            <% for (TransactionRow t : transactions) {
                ...
                if ("PURCHASE".equals(t.transactionType)) { typeBadgeClass = "badge-purchase"; typeLabel = "Mua hàng"; }
                else if ("REPAIR".equals(t.transactionType)) { typeBadgeClass = "badge-repair"; typeLabel = "Sửa chữa"; }
                else if ("IMPORT".equals(t.transactionType)) { typeBadgeClass = "badge-import"; typeLabel = "Nhập kho"; }
                String itemBadgeClass = "PART".equals(t.itemType) ? "badge-part" : "badge-equipment";
                String itemLabel      = "PART".equals(t.itemType) ? "Linh kiện" : "Thiết bị";
            %>
            <tr>
        ...
        <div class="pagination">
            <span class="paging-info">Hiển thị <%= transactions.size() %> / <%= total %> giao dịch — Trang <%= currentPage %>/<%= totalPages %></span>
            <div class="page-btns">
