...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .topbar-title{font-size:1.15rem;font-weight:800;color:var(--text-h);letter-spacing:-.3px}
        .topbar-sub{color:var(--text-s);font-size:.78rem;margin-top:2px}
        .topbar-badge{display:inline-flex;align-items:center;gap:7px;padding:7px 14px;background:var(--primary-light);border:1px solid rgba(99,102,241,0.25);border-radius:20px;color:var(--primary-2);font-size:.78rem;font-weight:600;}
        ...
        .tab{display:inline-flex;align-items:center;gap:8px;padding:8px 16px;border-radius:10px;font-size:.81rem;font-weight:600;text-decoration:none;transition:all .18s;border:1.5px solid var(--border-light);background:var(--bg-card);color:var(--text-m);white-space:nowrap;}
        .tab:hover{color:var(--text-b);background:#f3f4f6;border-color:#d1d5db;}
        .tab .cnt{padding:2px 7px;border-radius:20px;font-size:.68rem;font-weight:700;background:var(--border-light2);color:var(--text-s);}

        .tab-all.active      {background:var(--primary-light);border-color:rgba(99,102,241,0.4);color:var(--primary-2);}
        .tab-all.active .cnt {background:rgba(99,102,241,0.2);color:var(--primary-2);}
        .tab-purchase.active      {background:#d1fae5;border-color:#a7f3d0;color:#065f46;}
        .tab-purchase.active .cnt {background:#bbf7d0;color:#065f46;}
        .tab-repair.active        {background:#fef3c7;border-color:#fde68a;color:var(--amber);}
        .tab-repair.active .cnt   {background:#fde68a;color:var(--amber);}
        .tab-import.active        {background:#ede9fe;border-color:#c4b5fd;color:var(--purple);}
        .tab-import.active .cnt   {background:#ddd6fe;color:var(--purple);}

        /* ── FILTER BAR ── */
        .filter-bar{background:var(--bg-card);border:1.5px solid var(--border-light);border-radius:14px;padding:14px 16px;margin-bottom:14px;display:flex;gap:10px;align-items:center;flex-wrap:wrap;animation:cardIn .4s ease both;}
        .filter-bar input,
        .filter-bar select{padding:8px 12px;background:#fff;border:1.5px solid var(--border-light);border-radius:9px;color:var(--text-b);font-size:.82rem;font-family:inherit;outline:none;transition:all .2s;}
        .filter-bar input::placeholder{color:var(--text-s);}
        .filter-bar input:focus,
        .filter-bar select:focus{border-color:rgba(79,70,229,0.4);background:#faf9ff;box-shadow:0 0 0 3px rgba(79,70,229,0.07);}
        ...
        .btn-search{background:var(--primary-light);color:var(--primary-2);border:1.5px solid rgba(99,102,241,0.35);}
        .btn-search:hover{background:rgba(99,102,241,0.2);border-color:rgba(99,102,241,0.5);}
        .btn-reset{background:#fee2e2;color:var(--red);border:1.5px solid #fca5a5;}
        ...
        /* ── CARD / TABLE ── */
        .card{background:var(--bg-card);border:1.5px solid var(--border-light);border-radius:16px;overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,0.05);animation:cardIn .5s .1s ease both;}
        .card-hd{display:flex;justify-content:space-between;align-items:center;padding:13px 18px;border-bottom:1px solid var(--border-light2);background:#fafbff;}
        ...
        .total-badge{font-size:.72rem;color:var(--text-s);background:var(--border-light2);border:1px solid var(--border-light);padding:3px 10px;border-radius:20px;}

        table{width:100%;border-collapse:collapse;font-size:.79rem;}
        thead tr{background:#fafbff;}
        th{padding:9px 14px;text-align:left;color:var(--text-s);font-weight:700;font-size:.64rem;text-transform:uppercase;letter-spacing:.9px;border-bottom:1px solid var(--border-light2);}
        td{padding:10px 14px;border-bottom:1px solid var(--border-light2);vertical-align:middle;color:var(--text-b);}
        tr:last-child td{border-bottom:none}
        tbody tr{transition:background .12s;}
        tbody tr:hover td{background:#f7f8ff;}

        /* badges */
        .b{display:inline-flex;align-items:center;gap:4px;padding:3px 9px;border-radius:20px;font-size:.68rem;font-weight:700;white-space:nowrap;}
        .b-purchase{background:#d1fae5;color:#065f46;border:1px solid #a7f3d0;}
        .b-repair  {background:#fef3c7;color:#92400e;border:1px solid #fde68a;}
        .b-import  {background:#ede9fe;color:#5b21b6;border:1px solid #c4b5fd;}
        .b-other   {background:var(--border-light2);color:var(--text-s);border:1px solid var(--border-light);}
        .b-part     {background:var(--primary-light);color:var(--primary-2);border:1px solid rgba(99,102,241,0.3);}
        .b-equipment{background:#e0f2fe;color:var(--info);border:1px solid #bae6fd;}

        .td-id      {color:var(--text-s);font-size:.75rem;}
        .td-item    {font-weight:700;color:var(--text-h);font-size:.82rem;}
        .td-serial  {font-family:'Courier New',monospace;font-size:.78rem;color:var(--primary-2);font-weight:600;}
        .td-customer{font-weight:600;color:var(--text-b);font-size:.8rem;}
        .td-order   {font-family:'Courier New',monospace;font-size:.75rem;color:var(--green);font-weight:700;}
        .td-ref     {font-family:'Courier New',monospace;font-size:.75rem;color:var(--amber);font-weight:700;}
        .td-by      {font-size:.78rem;color:var(--text-b);}
        .td-note    {color:var(--text-s);font-size:.75rem;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
        .td-time    {color:var(--text-s);font-size:.75rem;white-space:nowrap;}
        .td-dash    {color:var(--text-s);}

        /* pagination */
        .pagination{display:flex;justify-content:space-between;align-items:center;padding:13px 18px;border-top:1px solid var(--border-light2);}
        .paging-info{color:var(--text-s);font-size:.75rem;}
        .page-btns{display:flex;gap:5px;}
        .page-btn{padding:6px 12px;border-radius:8px;background:var(--bg-card);border:1.5px solid var(--border-light);font-size:.78rem;color:var(--text-m);cursor:pointer;text-decoration:none;transition:all .18s;font-family:inherit;}
        .page-btn.active{background:var(--primary);border-color:var(--primary);color:#fff;font-weight:700;}
        .page-btn:hover:not(.active):not(.disabled){background:var(--primary-light);border-color:rgba(99,102,241,0.4);color:var(--primary-2);}
