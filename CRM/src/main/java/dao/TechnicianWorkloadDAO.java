...
public class TechnicianWorkloadDAO {
    ...
    /**
     * Lấy TẤT CẢ technician đang active, kể cả chưa có record trong
     * technician_workload. Dùng LEFT JOIN từ users → technician_workload để
     * không bỏ sót ai.
     */
    ...
    private TechnicianWorkload mapRow(ResultSet rs) throws SQLException {
        ...
        if (la != null) {
            tw.setLastAssignedDate(la.toLocalDateTime());
        }
        try {
            tw.setTechnicianName(rs.getString("technician_name"));
        } catch (Exception ignored) {
        }
        try {
            tw.setTechnicianEmail(rs.getString("technician_email"));
        } catch (Exception ignored) {
        }
        try {
            tw.setAvatarUrl(rs.getString("avatar_url"));
        } catch (Exception ignored) {
        }
        return tw;
    }
