...
public class WorkAssignmentDAO {
    ...
    public int create(WorkAssignment wa) throws Exception {
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            if (wa.getEstimatedDuration() != null) ps.setBigDecimal(4, wa.getEstimatedDuration());
    ...
    private WorkAssignment mapRow(ResultSet rs) throws SQLException {
        ...
        if (ad != null) wa.setAssignmentDate(ad.toLocalDateTime());
        ...
        try { int rid = rs.getInt("request_id"); if (!rs.wasNull()) wa.setRequestId(rid); } catch (Exception ignored) {}
        try { wa.setTaskStatus(rs.getString("task_status")); }     catch (Exception ignored) {}
        try { wa.setRequestCode(rs.getString("request_code")); }   catch (Exception ignored) {}
        try { wa.setRequestTitle(rs.getString("request_title")); } catch (Exception ignored) {}
        try { wa.setTechnicianName(rs.getString("technician_name")); } catch (Exception ignored) {}
        return wa;
    }
}