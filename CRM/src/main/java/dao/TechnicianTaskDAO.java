...
/**
 * DAO dành riêng cho Technician.
 * Dùng đúng tên bảng/cột của project mới:
 *   work_tasks   : id, request_id, technician_id, task_type, task_details, status, created_at
 *   work_assignments: id, task_id, assigned_by, assigned_to, estimated_duration, required_skills, priority, created_at
 *   service_requests: id, request_code, customer_id, contract_id, title, description, priority, status, assigned_to, assigned_at
 *   contracts    : id, contract_code, contract_type, status
 *   users        : id, full_name, address_full
 */
public class TechnicianTaskDAO {
    ...
    public boolean updateStatus(int taskId, int technicianId, String newStatus) throws Exception {
        String sql = "UPDATE work_tasks SET status = ? WHERE id = ? AND technician_id = ?";
        try (Connection con = DBConnection.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    private TaskRow mapRow(ResultSet rs) throws SQLException {
        ...
        try { row.requestCode    = rs.getString("request_code");    } catch (Exception ignored) {}
        try { row.requestTitle   = rs.getString("request_title");   } catch (Exception ignored) {}
        try { row.requestStatus  = rs.getString("request_status");  } catch (Exception ignored) {}
        try { row.priority       = rs.getString("priority");        } catch (Exception ignored) {}
        try { row.customerName   = rs.getString("customer_name");   } catch (Exception ignored) {}
        try { row.customerPhone  = rs.getString("customer_phone");  } catch (Exception ignored) {}
        try { row.customerAddress= rs.getString("customer_address");} catch (Exception ignored) {}
        try { row.contractCode   = rs.getString("contract_code");   } catch (Exception ignored) {}
        try { row.contractType   = rs.getString("contract_type");   } catch (Exception ignored) {}
        try { int cid = rs.getInt("contract_id"); if(!rs.wasNull()) row.contractId = cid; } catch (Exception ignored) {}
        try { row.assignPriority = rs.getString("assign_priority"); } catch (Exception ignored) {}
        try { row.requiredSkills = rs.getString("required_skills"); } catch (Exception ignored) {}
        try {
            Timestamp at = rs.getTimestamp("assigned_at");
            if (at != null) row.assignedAt = at.toLocalDateTime();
