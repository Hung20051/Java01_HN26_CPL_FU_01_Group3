...
public class FinanceDAO {
    ...
    public List<FinanceRow> getRows(String type, String fromDate, String toDate,
        ...
        if (type != null && !type.isBlank()) {
            sql.append(" AND inv.invoice_type = ?"); params.add(type);
        }
        if (fromDate != null && !fromDate.isBlank()) {
            sql.append(" AND DATE(p.created_at) >= ?"); params.add(fromDate);
        }
        if (toDate != null && !toDate.isBlank()) {
            sql.append(" AND DATE(p.created_at) <= ?"); params.add(toDate);
        }
    ...
    public int countRows(String type, String fromDate, String toDate, String keyword) throws Exception {
        StringBuilder sql = new StringBuilder("""
            ...
            """);
        List<Object> params = new ArrayList<>();
        if (type != null && !type.isBlank())     { sql.append(" AND inv.invoice_type = ?");          params.add(type); }
        if (fromDate != null && !fromDate.isBlank()) { sql.append(" AND DATE(p.created_at) >= ?");   params.add(fromDate); }
        if (toDate   != null && !toDate.isBlank())   { sql.append(" AND DATE(p.created_at) <= ?");   params.add(toDate); }
        if (keyword  != null && !keyword.isBlank()) {
            sql.append(" AND (u.full_name LIKE ? OR inv.invoice_code LIKE ? OR p.payment_code LIKE ?)");
            String kw = "%" + keyword.trim() + "%";
            params.add(kw); params.add(kw); params.add(kw);
