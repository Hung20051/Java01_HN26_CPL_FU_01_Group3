...
public class PartDAO {
    ...
    // PART TYPES
    // ============================
    public List<PartType> findAllTypes(String keyword, String categoryId,
    ...
    public int countTypes(String keyword, String categoryId) throws SQLException {
        StringBuilder sql = new StringBuilder(
            "SELECT COUNT(*) FROM part_types pt "
            + "JOIN categories c ON pt.category_id = c.id WHERE 1=1"
        );
        List<Object> params = new ArrayList<>();
        if (keyword != null && !keyword.trim().isEmpty()) {
            sql.append(" AND (pt.name LIKE ? OR pt.description LIKE ?)");
    ...
    public int insertType(PartType pt) throws SQLException {
        String sql = "INSERT INTO part_types "
                   + "(name, category_id, description, unit_price, image_url, updated_by) "
                   + "VALUES (?,?,?,?,?,?)";
        try (Connection c = DBConnection.getConnection();
            ...
            ps.setString(5, pt.getImageUrl());
            ps.setInt   (6, pt.getUpdatedBy());
            ps.executeUpdate();
    ...
    public void updateType(PartType pt) throws SQLException {
        String sql = "UPDATE part_types "
                   + "SET name=?, category_id=?, description=?, unit_price=?, image_url=?, updated_by=? "
                   + "WHERE id=?";
        try (Connection c = DBConnection.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, pt.getName());
    ...
    public void deleteType(int id) throws SQLException {
        try (Connection c = DBConnection.getConnection()) {
            PreparedStatement ps1 = c.prepareStatement(
                "DELETE FROM part_units WHERE part_type_id = ?");
    ...
    public void insertUnits(int partTypeId, int quantity, int performedBy) throws SQLException {
        ...
        String sqlTxn  = "INSERT INTO inventory_transactions "
                       + "(item_type, item_unit_id, action, performed_by, note) "
                       + "VALUES ('PART', ?, 'IMPORT', ?, 'Nhập kho mới')";
        try (Connection c = DBConnection.getConnection()) {
    ...
    /**
     * Xóa ngẫu nhiên N units có status=AVAILABLE của một part type.
     * Chỉ xóa AVAILABLE, không động INUSE/FAULTY/RETIRED.
     * @return số unit thực sự đã xóa (có thể < quantity nếu không đủ AVAILABLE)
     */
    public int deleteAvailableUnits(int partTypeId, int quantity) throws SQLException {
        String sqlFind = "SELECT id FROM part_units "
                       + "WHERE part_type_id = ? AND status = 'AVAILABLE' "
                       + "ORDER BY RAND() LIMIT ?";
        String sqlDel  = "DELETE FROM part_units WHERE id = ?";
        ...
        try (Connection c = DBConnection.getConnection()) {
            ...
            try {
                ...
                PreparedStatement psDel = c.prepareStatement(sqlDel);
                while (rs.next()) {
                    psDel.setInt(1, rs.getInt("id"));
                    psDel.addBatch();
                    deleted++;
                }
                if (deleted > 0) psDel.executeBatch();
                c.commit();
            } catch (Exception e) {
                c.rollback();
                throw e;
    ...
    public List<PartUnit> findUnitsByTypeId(int partTypeId) throws SQLException {
        String sql = "SELECT pu.*, pt.name as part_type_name FROM part_units pu "
                   + "JOIN part_types pt ON pu.part_type_id = pt.id "
                   + "WHERE pu.part_type_id = ? ORDER BY pu.id";
    ...
    public Map<String, Integer> getDashboardStats() throws SQLException {
        ...
        String sql = "SELECT "
            ...
            + "(SELECT COUNT(DISTINCT pt.id) FROM part_types pt "
            + " JOIN part_units pu ON pu.part_type_id = pt.id "
            + " WHERE (SELECT COUNT(*) FROM part_units pu2 "
            + "        WHERE pu2.part_type_id = pt.id AND pu2.status='AVAILABLE') <= 2 "
            + " AND   (SELECT COUNT(*) FROM part_units pu2 "
            + "        WHERE pu2.part_type_id = pt.id AND pu2.status='AVAILABLE') > 0) as low_stock";
        try (Connection c = DBConnection.getConnection();
             PreparedStatement ps = c.prepareStatement(sql)) {
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
    ...
    public void insertUnitsExport(int partTypeId, int quantity, int performedBy, int invoiceId)
        ...
        try (Connection c = DBConnection.getConnection()) {
            ...
            try {
                PreparedStatement psFind = c.prepareStatement(sqlFind);
                psFind.setInt(1, partTypeId); psFind.setInt(2, quantity);
                ResultSet rs = psFind.executeQuery();
                PreparedStatement psUpd = c.prepareStatement(sqlUpd);
                PreparedStatement psTxn = c.prepareStatement(sqlTxn);
                while (rs.next()) {
                    int unitId = rs.getInt("id");
                    psUpd.setInt(1, unitId); psUpd.addBatch();
                    psTxn.setInt(1, unitId); psTxn.setInt(2, performedBy);
                    psTxn.setInt(3, invoiceId); psTxn.addBatch();
                }
                psUpd.executeBatch(); psTxn.executeBatch();
                c.commit();
        ...
        try (Connection c = DBConnection.getConnection()) {
            ...
            try {
                ...
                if (rs.next()) {
                    ...
                    PreparedStatement psTxn = c.prepareStatement(sqlTxn);
                    psTxn.setInt(1, unitId); psTxn.setInt(2, performedBy);
                    psTxn.setInt(3, invoiceId); psTxn.executeUpdate();
                }
                c.commit();
            } catch (Exception e) { c.rollback(); throw e; }
            finally { c.setAutoCommit(true); }
