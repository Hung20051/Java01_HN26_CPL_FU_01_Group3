...
public class PartDAO {
    ...
    /**
     * Xóa ngẫu nhiên N units có status=AVAILABLE của một part type. Chỉ xóa
     * AVAILABLE, không động INUSE/FAULTY/RETIRED.
     *
    ...
    private PartUnit mapUnitRow(ResultSet rs) throws SQLException {
        ...
        Timestamp ca = rs.getTimestamp("created_at");
        if (ca != null) {
            pu.setCreatedAt(ca.toLocalDateTime());
        }
        Timestamp ua = rs.getTimestamp("updated_at");
        if (ua != null) {
            pu.setUpdatedAt(ua.toLocalDateTime());
        }
        return pu;
    }
