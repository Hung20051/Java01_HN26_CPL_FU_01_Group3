...
public class ShopDAO {
    ...
    StringBuilder sql = new StringBuilder(
            "SELECT pt.id, pt.name, c.name AS category_name, pt.description, pt.unit_price, pt.image_url, "
    ...
    try (Connection c = DBConnection.getConnection();
        ...
        while (rs.next()) {
            ...
            item.imageUrl     = rs.getString("image_url");   // ← THÊM
            list.add(item);
        }
    }
    return list;
    ...
    public List<ShopItem> getAvailableEquipment(String keyword, String categoryId,
        ...
        throws SQLException {
    StringBuilder sql = new StringBuilder(
            "SELECT et.id, et.model AS name, c.name AS category_name, et.description, et.unit_price, et.image_url, "
   ...
   public ShopItem getPartById(int partTypeId) throws SQLException {
    String sql = "SELECT pt.id, pt.name, c.name AS category_name, pt.description, pt.unit_price, pt.image_url, "
            + "SUM(CASE WHEN pu.status='AVAILABLE' THEN 1 ELSE 0 END) AS available_qty "
...
public ShopItem getEquipmentById(int equipTypeId) throws SQLException {
    String sql = "SELECT et.id, et.model AS name, c.name AS category_name, et.description, et.unit_price, et.image_url, "
            + "SUM(CASE WHEN eu.status='AVAILABLE' THEN 1 ELSE 0 END) AS available_qty "
