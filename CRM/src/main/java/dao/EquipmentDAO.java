...
public class EquipmentDAO {
    ...
    public int insertType(EquipmentType et) throws SQLException {
        String sql = "INSERT INTO equipment_types (model, category_id, description, unit_price, image_url, updated_by) VALUES (?,?,?,?,?,?)";
    ...
    public void updateType(EquipmentType et) throws SQLException {
        String sql = "UPDATE equipment_types SET model=?, category_id=?, description=?, unit_price=?, image_url=?, updated_by=? WHERE id=?";
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(sql)) {
            ps.setString(1, et.getModel());
            ps.setInt(2, et.getCategoryId());
            ps.setString(3, et.getDescription());
    ...
    public int deleteAvailableUnits(int equipmentTypeId, int qty) throws SQLException {
        ...
        try (Connection c = DBConnection.getConnection(); PreparedStatement ps = c.prepareStatement(selectSql)) {
            ps.setInt(1, equipmentTypeId);
            ps.setInt(2, qty);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    private EquipmentType mapTypeRow(ResultSet rs) throws SQLException {
        ...
        et.setImageUrl(rs.getString("image_url"));
