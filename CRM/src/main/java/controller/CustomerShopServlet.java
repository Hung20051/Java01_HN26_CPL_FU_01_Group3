...
public class CustomerShopServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (action.isEmpty()) action = "parts";
        try {
            switch (action) {
                case "parts":        showParts(req, resp);        break;
                case "equipment":    showEquipment(req, resp);    break;
                case "cart":         showCart(req, resp);         break;
                case "checkout":     showCheckout(req, resp);     break;
                case "orderSuccess": showOrderSuccess(req, resp); break;
                case "vnpay_gateway":
                    ...
                    if (invId == null) { resp.sendRedirect(req.getContextPath() + "/customerShop?action=cart"); return; }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            switch (action) {
                case "addToCart":     handleAddToCart(req, resp, me);    break;
                case "updateCart":    handleUpdateCart(req, resp);       break;
                case "removeCart":    handleRemoveCart(req, resp);       break;
                case "clearCart":     handleClearCart(req, resp);        break;
                case "checkout":      handleCheckout(req, resp, me);     break;
                case "vnpay_confirm": handleVnpayConfirm(req, resp);     break;
                case "vnpay_cancel":  handleVnpayCancel(req, resp);      break;
                default: resp.sendRedirect(req.getContextPath() + "/customerShop?action=cart");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void showParts(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        req.setAttribute("keyword", kw); req.setAttribute("categoryId", catId); req.setAttribute("sortBy", sort);
        req.setAttribute("currentPage", page); req.setAttribute("totalPages", (int)Math.ceil((double)total/PAGE_SIZE));
        req.setAttribute("total", total); req.setAttribute("cartCount", getCartCount(req));
    ...
    private void showCart(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        req.getSession().removeAttribute("shopFlashSuccess"); req.getSession().removeAttribute("shopFlashError");
        req.setAttribute("cartList", new ArrayList<>(cart.values())); req.setAttribute("grandTotal", grand);
        req.setAttribute("cartCount", cart.size()); req.setAttribute("flashSuccess", fs); req.setAttribute("flashError", fe);
    ...
    private void showCheckout(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        Map<String, CartItem> cart = getCart(req);
        if (cart.isEmpty()) { resp.sendRedirect(req.getContextPath() + "/customerShop?action=cart"); return; }
        double sub = cart.values().stream().mapToDouble(CartItem::getSubtotal).sum();
        double tax = sub * 0.10;
        req.setAttribute("cartList", new ArrayList<>(cart.values()));
        req.setAttribute("subtotal", sub); req.setAttribute("tax", tax); req.setAttribute("grandTotal", sub+tax);
    ...
    private void showOrderSuccess(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        req.getSession().removeAttribute("lastOrderCode"); req.getSession().removeAttribute("lastPayMethod");
    ...
    private void handleAddToCart(HttpServletRequest req, HttpServletResponse resp, User me) throws Exception {
        ...
        if (si == null || si.availableQty <= 0) {
            ...
            resp.sendRedirect(req.getContextPath() + "/customerShop?action=" + backAction); return;
        }
    ...
    private void handleUpdateCart(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        String key = nvl(req.getParameter("key")); int qty = parseInt(req.getParameter("quantity"), 1);
        Map<String, CartItem> cart = getCart(req);
        if (cart.containsKey(key)) {
    ...
    private void handleCheckout(HttpServletRequest req, HttpServletResponse resp, User me) throws Exception {
        ...
        for (CartItem ci : cart.values()) {
            ...
            ii.setItemType(ci.getItemType()); ii.setQuantity(ci.getQuantity());
    ...
    private void handleVnpayConfirm(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        if (payId == null) { resp.sendRedirect(req.getContextPath() + "/customerShop?action=parts"); return; }
    ...
    private Invoice createShopInvoice(int customerId, List<InvoiceItem> items, int createdBy) throws Exception {
        ...
        try (Connection c = util.DBConnection.getConnection()) {
            ...
            try {
                ...
                ps.setString(1, code); ps.setInt(2, customerId);
                ps.setBigDecimal(3, sub); ps.setBigDecimal(4, taxPct);
                ps.setBigDecimal(5, taxAmt); ps.setBigDecimal(6, total);
                ps.setDate(7, due); ps.setString(8, "Đơn mua hàng online"); ps.setInt(9, createdBy);
                ps.executeUpdate();
                int invId = -1;
                ...
                for (InvoiceItem it : items) {
                    psi.setInt(1, invId); psi.setString(2, it.getItemName());
                    psi.setString(3, it.getItemType()); psi.setInt(4, it.getQuantity());
                    psi.setBigDecimal(5, it.getUnitPrice()); psi.setBigDecimal(6, it.getTotalPrice());
                    psi.addBatch();
                }
                psi.executeBatch();
                c.commit();
    ...
    private void deductInventory(Map<String, CartItem> cart, int performedBy, int invoiceId) throws Exception {
        try (Connection c = util.DBConnection.getConnection()) {
            c.setAutoCommit(false);
            try {
                for (CartItem ci : cart.values()) {
                    ...
                    if (isPart) psF.setInt(2, ci.getQuantity());
                    ...
                    while (rs.next()) {
                        ...
                        psTxn.setString(1, ci.getItemType()); psTxn.setInt(2, uid);
                        psTxn.setInt(3, performedBy); psTxn.setInt(4, invoiceId);
                        psTxn.executeUpdate();
                    }
                }
                c.commit();
    ...
    private int getCartCount(HttpServletRequest req) { return getCart(req).size(); }
