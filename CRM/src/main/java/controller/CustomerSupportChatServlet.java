    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if ("typing".equals(action)) {
                String cidParam = req.getParameter("customerId");
                if (cidParam != null) {
                    dao.setTyping(me.getId(), Integer.parseInt(cidParam));
                }
                resp.getWriter().write("{\"ok\":true}");
                return;
            }
            ...
            if ("stopTyping".equals(action)) {
                String cidParam = req.getParameter("customerId");
                if (cidParam != null) {
                    dao.clearTyping(me.getId(), Integer.parseInt(cidParam));
                }
                resp.getWriter().write("{\"ok\":true}");
                return;
            }
            ...
            if (cidParam == null) {
                resp.getWriter().write("{\"success\":false,\"error\":\"missing_customer\"}");
                return;
            }
